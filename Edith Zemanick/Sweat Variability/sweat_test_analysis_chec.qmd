...
home_dir <- switch(Sys.info()["sysname"],
  ...
  "Linux" = "/home/timvigers/OneDrive/Vigers/CFEdith Zemanick/Sweat Variability"
)
...
```{r results='asis'}
t1 <- tableby(
  modqual ~ sex + kwt(age, "N", "Nmiss2", "medianq1q3") + agecat_txt +
    race_txt + genotype_group + swcavg + swhcavgpm + swsavg + swhsavgpm,
  data = chec, cat.test = "fe", simulate.p.value = TRUE
)
...
summary(tableby(modqual ~ post_ratio + post_ratio_1.2,
  data = chec,
  control = list(
...
plot_df$modqual <- factor(plot_df$modqual,
  levels = c(
    "Pre-Modulator", "Lumacaftor / Ivacaftor", "Tezacaftor / Ivacaftor",
    "Ivacaftor", "Elexacaftor / Tezacaftor / Ivacaftor"
  )
)
ggplot(plot_df, aes(x = modqual, y = ratio)) +
  geom_boxplot() +
  ylab("Na:Cl") +
  theme_bw() +
