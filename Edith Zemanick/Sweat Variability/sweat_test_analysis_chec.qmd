...
```{r data cleaning}
# Import CHEC data
chec <- read.csv("./Data_Raw/CHEC sweat sodium and chloride 2024-04-19.csv",
  na.strings = c("", "NA")
)
ethnicity <- read.csv("./Data_Raw/CHEC sweat sodium and chloride 2025-07-30.csv",
  na.strings = c("", "NA")
)
chec <- left_join(chec, ethnicity %>% select(subject, crfethnicity_txt),
  by = join_by(subject)
)
...
chec$crfethnicity_txt <- factor(chec$crfethnicity_txt,
...
t1 <- tableby(
  modqual ~ sex + kwt(age) + agecat_txt +
    race_txt + crfethnicity_txt + genotype_group + swcavg + swhcavgpm + swsavg +
    swhsavgpm + pre_ratio + post_ratio,
