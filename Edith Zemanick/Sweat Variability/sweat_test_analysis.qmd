...
geno_n <- df |>
  filter(
    Diagnosis == "Cystic Fibrosis" & `Genotype Risk` %in% c("High", "Low")
  ) |>
...
clp_g <- df |>
  ...
  mutate(Diagnosis = droplevels(Diagnosis)) |>
  ggplot(aes(x = `Genotype Risk`, y = Cl)) +
  geom_boxplot() +
  geom_label(
    data = geno_n,
...
nap_g <- df |>
  ...
  mutate(Diagnosis = droplevels(Diagnosis)) |>
  ggplot(aes(x = `Genotype Risk`, y = Na)) +
  geom_boxplot() +
  geom_label(
    data = geno_n,
    aes(x = `Genotype Risk`, y = pos, label = paste("n =", n))
...
ratiop_g <- df |>
  ...
  mutate(Diagnosis = droplevels(Diagnosis)) |>
  ggplot(aes(x = `Genotype Risk`, y = `Na:Cl`)) +
  geom_boxplot() +
  geom_label(
    data = geno_n,
    aes(x = `Genotype Risk`, y = pos, label = paste("n =", n))
...
To compare the utility of Na:Cl ratio vs Cl alone, we plotted the Na:Cl ratio for those misclassified by Cl. The Cl threshold that maximized the Youden index when distinguishing Non-CF from CFTR-RD or CRMS was 27.75 (Area under the curve: 0.9021 [95% CI: 0.8539-0.9504 (DeLong)]). So, people with Cl < 27.75 and a diagnosis of CFTR-RD or CRMS were misclassified as Non-CF, and people with Cl >= 27.75 and a diagnosis of Non-CF were misclassified as CFTR-RD or CRMS. A total of 3 people with CFTR-RD or CRMS were missed based on this cutoff, and there did not appear to be a Na:Cl ratio that would clearly separate the two groups. The results were the same when using a threshold of 30 (4 people with CFTR-RD or CRMS were missed)
...
clp_d <- ggplot(
  df |>
    filter(
      (Cl < 27.75 & Diagnosis == "CFTR-RD or CRMS") |
        (Cl >= 27.75 & Diagnosis == "Non-CF")
    ) |>
    mutate(Diagnosis = droplevels(Diagnosis)),
  aes(x = Diagnosis, y = `Na:Cl`)
) +
  geom_point() +
  theme_bw() +
  ggtitle("Cl Threshold of 27.75")
clp_d
clp_d <- ggplot(
  df |>
    filter(
      (Cl < 30 & Diagnosis == "CFTR-RD or CRMS") |
        (Cl >= 30 & Diagnosis == "Non-CF")
    ) |>
    mutate(Diagnosis = droplevels(Diagnosis)),
  aes(x = Diagnosis, y = `Na:Cl`)
) +
...
# Non-CF vs. CRMS
non_vs_crms = df |>
  filter(Diagnosis %in% c("Non-CF", "CFTR-RD or CRMS")) |>
  mutate(Diagnosis = droplevels(Diagnosis))
mod1 = glm(Diagnosis ~ Cl, data = non_vs_crms, family = "binomial")
mod2 = glm(Diagnosis ~ `Na:Cl`, data = non_vs_crms, family = "binomial")
mod3 = glm(Diagnosis ~ Na + Cl, data = non_vs_crms, family = "binomial")
roc_cl <- roc(Diagnosis ~ fitted(mod1), data = non_vs_crms)
roc_ratio <- roc(Diagnosis ~ fitted(mod2), data = non_vs_crms)
roc_na_cl <- roc(Diagnosis ~ fitted(mod3), data = non_vs_crms)
roc.test(roc_cl, roc_ratio)
roc.test(roc_cl, roc_na_cl)
# CRMS vs. CF
crms_vs_cf = df |>
  filter(Diagnosis %in% c("CFTR-RD or CRMS", "Cystic Fibrosis")) |>
  mutate(Diagnosis = droplevels(Diagnosis))
mod1 = glm(Diagnosis ~ Cl, data = crms_vs_cf, family = "binomial")
mod2 = glm(Diagnosis ~ `Na:Cl`, data = crms_vs_cf, family = "binomial")
mod3 = glm(Diagnosis ~ Na + Cl, data = crms_vs_cf, family = "binomial")
roc_cl <- roc(Diagnosis ~ fitted(mod1), data = crms_vs_cf)
roc_ratio <- roc(Diagnosis ~ fitted(mod2), data = crms_vs_cf)
roc_na_cl <- roc(Diagnosis ~ fitted(mod3), data = crms_vs_cf)
roc.test(roc_cl, roc_ratio)
roc.test(roc_cl, roc_na_cl)
```

We also compared the ability of 3 logistic regression models to distinguish between Non-CF vs. CFTR-RD or CRMS and CFTR-RD or CRMS vs. Cystic Fibrosis. Model 1 used only Cl as a predictor, model 2 used only Na:Cl as a predictor, and model 3 used Na, Cl, and an interaction term between Na and Cl as a predictor. 

When classifying participants as Non-CF vs. CFTR-RD or CRMS, chloride alone performed significantly better than Na:Cl ratio (AUC 0.90 vs. 0.78, p = 0.03) and equally as well as the interaction model (AUC 0.90 vs. 0.91, p = 0.9).

When classifying participants as CFTR-RD or CRMS vs. Cystic Fibrosis, chloride alone performed significantly better than Na:Cl ratio (AUC 0.99 vs. 0.86, p = 0.01) and equally as well as the interaction model (AUC 0.99 vs. 0.99, p = 0.05).
