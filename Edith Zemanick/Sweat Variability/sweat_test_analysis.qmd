...
geno_tab = xtabs(
  ~ `Detailed Genotype` + `Genotype Risk` + Diagnosis,
  data = df,
  addNA = T
) |>
  data.frame() |>
  ...
  select(-Diagnosis) |>
  pivot_wider(names_from = Genotype.Risk, values_from = Freq)
categories = c("Not CF", "Low", "High", "Unknown")
geno_tab$Category = apply(geno_tab[, categories], 1, function(r) {
  w = which(r != 0)
  if (length(w) > 0) {
    return(categories[w])
  } else {
...
geno_tab$Count = rowSums(geno_tab[, c("Not CF", "Low", "High", "Unknown")])
geno_tab$Percentage = scales::percent(geno_tab$Count / sum(geno_tab$Count))
geno_tab$`Num. CF Participants` = paste0(
  geno_tab$Count,
  " (",
  geno_tab$Percentage,
  ")"
...
geno_tab = geno_tab |>
  ...
  select(Detailed.Genotype, `Num. CF Participants`, Category) |>
  rename(`Detailed Genotype` = Detailed.Genotype)
write.csv(
  geno_tab,
  file = "./Dissemination/Genotypes.csv",
  row.names = F,
  na = ""
)
```
