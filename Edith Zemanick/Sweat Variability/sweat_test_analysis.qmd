...
roc.test(roc_cl, roc_ratio)
# Run both ROC curves
mc_roc_ratio <- multiclass.roc(
  df$Diagnosis,
  df$`Na:Cl`,
  quiet = T,
  levels = c("Non-CF", "CFTR-RD or CRMS", "Cystic Fibrosis")
)
mc_roc_cl <- multiclass.roc(
  df$Diagnosis,
  df$Cl,
  quiet = T,
  levels = c("Non-CF", "CFTR-RD or CRMS", "Cystic Fibrosis")
)
# Get the people misclassified by Cl
non_vs_crms_cl_thresh = coords(mc_roc_cl$rocs[[1]], "best")$threshold
df$roc_cl_pred = ifelse(
  df$Cl >= non_vs_crms_cl_thresh,
  "CFTR-RD or CRMS",
  "Non-CF"
)
# Same for Na:Cl
non_vs_crms_ratio_thresh = coords(mc_roc_ratio$rocs[[1]], "best")$threshold
df$roc_ratio_pred = ifelse(
  df$`Na:Cl` >= non_vs_crms_ratio_thresh,
  "CFTR-RD or CRMS",
  "Non-CF"
)
# Compare them
non_vs_crms_comp = df |>
  filter(
    Diagnosis %in% c("Non-CF", "CFTR-RD or CRMS"),
    Diagnosis != roc_cl_pred
  ) |>
  select(Name, Diagnosis, roc_ratio_pred, roc_cl_pred)
non_vs_crms_comp$Diagnosis = droplevels(non_vs_crms_comp$Diagnosis)
```
