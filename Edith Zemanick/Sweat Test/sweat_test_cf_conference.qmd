---
title: "Using Sweat Na/Cl Ratio to Predict CFTR-RD and CRMS"
author: "Tim Vigers"
date: "today"
date-format: long
format: revealjs
...
```{r}
set.seed(1017)
cart_full <- rpart(Diagnosis ~ Na + Cl, data = cart_df, method = "class",
...
cart_non_cf_crms <- rpart(Diagnosis ~ Na + Cl, data = non_cf_crms,
                          method = "class")
rpart.plot(cart_non_cf_crms)
```
...
1. Additional work on the prediction approaches to see if we can improve the results in our current data. 
  ...
  - Support vector machines may allow for more flexeible partitioning of the Na/Cl space.

2. Validate final prediction models on new data.
...
---
nocite: |
  @jamesIntroductionStatisticalLearning2021, @robinPROCOpensourcePackage2011, @therneau1997introduction
---
