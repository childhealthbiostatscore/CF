...
# Get list of those multiple values
mult <- unique(df$PatientID[which(duplicated(df$PatientID))])
# Testing options (most variables are not normally distributed)
...
```{r results='asis'}
t1 <- tableby(Method ~ Sex + Age.at.Test.in.Years + Diagnosis + geno_risk + Amount + Cl + Na + K + `Na/Cl Ratio` + Arm,
...
```{r results='asis'}
t1 <- tableby(Diagnosis ~ Amount + Cl + Na + K + `Na/Cl Ratio` + Arm,
  data = t1_df,control = mycontrols,
)
summary(t1,pfootnote = T,
  labelTranslations =
...
```{r results='asis'}
t1 <- tableby(geno_risk ~ Amount + Cl + Na + K + `Na/Cl Ratio` + Arm,
  data = t1_df,control = mycontrols,
)
summary(t1,pfootnote = T,
  labelTranslations =
...
```{r results='asis'}
t1 <- tableby( ~ Amount + Cl + Na + K + `Na/Cl Ratio` + Arm,
...
1. What are "CL diff" and "NA diff" (e.g. in Table 4)? If it's one sweat collection per patient, is is the average difference between people?
