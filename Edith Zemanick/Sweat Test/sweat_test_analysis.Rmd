...
```{r results='asis'}
t1 <- tableby(Method ~ Sex + Age.at.Test.in.Years + Diagnosis + geno_risk + Amount + Cl + Na + K + Arm,
  data = t1_df
)
summary(t1,
  labelTranslations =
...
### Participants with more than one measure

```{r results='asis'}
t1 <- tableby(Method ~ Sex + Age.at.Test.in.Years + Diagnosis + geno_risk + Amount + Cl + Na + K + Arm,
  data = t1_df[t1_df$PatientID %in% mult, ]
)
summary(t1,
  labelTranslations =
    list(Age.at.Test.in.Years = "Age (Years)", geno_risk = "Genotype Severity")
