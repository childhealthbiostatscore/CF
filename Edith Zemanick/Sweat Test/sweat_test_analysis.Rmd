...
## By function
df$geno1_class <- get_cf_genotype_class_severity(df$Genotypes1,out = "class")
...
df$geno2_class <- get_cf_genotype_class_severity(df$Genotypes2,out = "class")
...
df$geno_class = factor(paste0(pmin(df$geno1_class,df$geno2_class),"/",
...
labels = list(Age.at.Test.in.Years = "Age (Years)", 
...
t1 <- tableby(Diagnosis ~ Sex + Age.at.Test.in.Years + fe(geno_risk) + fe(geno_class) + 
...
```{r results='asis'}
t1 <- tableby(Method ~ Sex + Age.at.Test.in.Years + Diagnosis + fe(geno_risk) + fe(geno_class) +
...
```{r results='asis'}
t1 <- tableby(Diagnosis ~ Amount + Cl + Na + K + `Na/Cl Ratio` + 
...
```{r results='asis'}
t1 <- tableby(geno_risk ~ Amount + Cl + Na + K + `Na/Cl Ratio` + 
                `Cl Diff. Between Left and Right Arm` + 
                `Na Diff. Between Left and Right Arm`,
  data = t1_df,control = mycontrols,
)
...
```{r results='asis'}
t1 <- tableby(geno_class ~ Amount + Cl + Na + K + `Na/Cl Ratio` + 
                `Cl Diff. Between Left and Right Arm` + 
                `Na Diff. Between Left and Right Arm`,
  data = t1_df,control = mycontrols,
)
...
ggplot(df, aes(x = geno_class, y = Na)) +
  geom_boxplot() +
  theme_bw() + xlab("Specific Genotype") +
  ggtitle("Sweat Sodium")
...
```{r results='asis'}
p <- paired(time ~ Diagnosis + Cl + Na + K + `Na/Cl Ratio` + 
              `Cl Diff. Between Left and Right Arm` + 
              `Na Diff. Between Left and Right Arm`,
...
```{r results='asis'}
p <- paired(time ~ Cl + Na + K + `Na/Cl Ratio`,signed.rank.exact = F,strata = geno_class,
            data = multiples %>% filter(Diagnosis == "Non-CF"), 
            id = PatientID,control = mycontrols)
summary(p,pfootnote = T,labelTranslations = labels)
```
