...
df$geno_risk = factor(df$geno_risk,
                      levels = c("Mild_Severe","Mild_Unknown","Not CF_Not CF",
                                 "Severe_Mild","Mild_Mild",
                                 "Severe_Severe","Severe_Unknown","Unknown_Severe",
                                 "Unknown_Unknown"),
                      labels = c("Low","Low","Not CF","Low","Low","High","Unknown",
                                 "Unknown","Unknown"))
...
```{r}
ggplot(df,aes(x = Diagnosis,y = Cl)) + geom_boxplot() + theme_bw()
```

```{r}
# Write groups of interest
write.csv(df %>% filter(Diagnosis == "Non-CF",Cl >= 60),na = "",
          file = "./Data_Cleaned/non_cf_cl_over_60.csv",row.names = F)
write.csv(df %>% filter(Diagnosis == "Non-CF",Cl >= 30 & Cl < 60),na = "",
          file = "./Data_Cleaned/non_cf_cl_30_59.csv",row.names = F)
write.csv(df %>% filter(Diagnosis == "Cystic Fibrosis",Cl < 60),na = "",
          file = "./Data_Cleaned/cf_cl_under_60.csv",row.names = F)
write.csv(df %>% filter(Diagnosis == "Cystic Fibrosis",Cl >= 30 & Cl < 60),na = "",
          file = "./Data_Cleaned/cf_cl_30_59.csv",row.names = F)
```
