...
paired_df = df %>% 
  ...
  drop_na(any_of(c("VitaminA_Retinol"))) %>%
...
summary(p,
        labelTranslations = list(VitaminA_Retinol = "Vitamin A Retinol",
                                 ...
                                 Vitamin.E.Alpha = "Vitamin E Alpha",
...
```{r}
# Set up data again
df = read.csv("./Edith Zemanick/CF Nutrition/Data_Cleaned/analysis_dataset.csv")
...
df = df %>% filter(!is.na(time) & time >= -3) %>% 
  select(Patient.ID,Age.at.Test..Years.,age_first,age_second,time,c1,c2,
...
piecewise_mod = function(outcome,data,diagnostics = F){
  ...
  data = data %>% select(Patient.ID,time,c1,c2,age_first,age_second,
                         all_of(outcome)) %>% drop_na(all_of(outcome))
  # Separate data for single and double mods
  single = data %>% filter(is.na(age_second))
  double = data %>% filter(!is.na(age_second))
  # Models
  f_single = as.formula(paste0(outcome,"~ time + c1 + (1|Patient.ID)"))
  m_single = lmer(f_single,data = single)
  f_double = as.formula(paste0(outcome,"~ time + c1 +c2 + (1|Patient.ID)"))
