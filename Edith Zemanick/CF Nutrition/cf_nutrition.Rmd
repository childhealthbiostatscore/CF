...
paired_df = df %>% 
  ...
  mutate(years = round(years)) %>%
  group_by(Patient.ID,years) %>%
  fill(all_of(nutrition_vars),.direction = "downup") %>%
  drop_na(any_of(c("VitaminA_Retinol","FEV1...pred"))) %>%
  group_by(Patient.ID,Before_After) %>%
  slice_min(abs(years)) %>% distinct() %>% filter(row_number()==1) 
...
p = paired(f,data = paired_df,id = Patient.ID,strata = first_mod)
summary(p,
...
df = read.csv("~/Dropbox/Work/CF/Edith Zemanick/CF Nutrition/Data_Cleaned/analysis_dataset.csv")
df$years = df$Age.at.Test..Years. - df$Age.at.Start..years.
t = df %>% group_by(Patient.ID) %>% summarise(n = sum(!is.na(VitaminA_Retinol)))
```

```{r}
outcome = "VitaminA_Retinol"
piecewise_mod = function(outcome,data){
  data = data %>% select(Patient.ID,years,all_of(outcome)) %>% drop_na()
  plot = ggplot(data,aes_string(x = "years",y = outcome,group = "Patient.ID")) + 
