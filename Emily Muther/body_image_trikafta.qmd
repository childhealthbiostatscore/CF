...
df %>%
  ...
  tbl_summary(
    by = Gender,
    type = list(c("FRStotal", "FRS_IdealBody", "FRS_ThinkyouLook") ~
      "continuous"),
    statistic = list(all_of(normal_dist) ~ "{mean} [{sd}]"),
...
df %>%
  ...
  mutate(PercentAdherence=as.numeric(sub("%","",as.character(PercentAdherence)))) %>%
  tbl_summary(
    by = "FRS_Category",
