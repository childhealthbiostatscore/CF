...
homa = df %>% select(Study.ID,Event.Name,Glucose.0.minute:Glucose.180.min,
                     c.peptide.0.minute:c.peptide.180.min)
glu_cols = colnames(df)[grep("glucose\\.\\d{1,}",tolower(colnames(df)))]
cpep_cols = colnames(df)[grep("c.peptide",tolower(colnames(df)))]
homa[,glu_cols] = lapply(homa[,glu_cols], function(c){
  c/18
})
homa[,cpep_cols] = lapply(homa[,cpep_cols], function(c){
  c = suppressWarnings(as.numeric(c))/1000
  c*0.33
})
