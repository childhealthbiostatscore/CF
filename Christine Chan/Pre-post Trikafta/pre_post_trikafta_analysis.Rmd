...
piecewise = function(outcome,data = df,simple = F){
  # Add spline column
...
```{r results='asis'}
meansg = piecewise(outcome = "average_sensor",df)
...
```{r results='asis'}
minsg = piecewise(outcome = "min_sensor",df)
ggplotly(minsg$plot)
...
```{r results='asis'}
maxsg = piecewise(outcome = "max_sensor",df)
ggplotly(maxsg$plot)
...
```{r results='asis'}
over120 = piecewise(outcome = "percent_time_over_120",df)
...
```{r results='asis'}
over140 = piecewise(outcome = "percent_time_over_140",df)
...
```{r results='asis'}
over200 = piecewise(outcome = "percent_time_over_200",df)
...
```{r results='asis'}
numover140 = piecewise(outcome = "avg_excur_over_140_per_day",df)
...
```{r results='asis'}
numover200 = piecewise(outcome = "avg_excur_over_200_per_day",df)
...
```{r results='asis'}
under70 = piecewise(outcome = "percent_time_under_70",df)
...
```{r results='asis'}
under60 = piecewise(outcome = "percent_time_under_60",df)
...
```{r results='asis'}
auc = piecewise(outcome = "average_auc_per_day",df)
ggplotly(auc$plot)
...
```{r results='asis'}
sd = piecewise(outcome = "standard_deviation",df)
ggplotly(sd$plot)
...
```{r results='asis'}
mage = piecewise(outcome = "r_mage",df)
ggplotly(mage$plot)
