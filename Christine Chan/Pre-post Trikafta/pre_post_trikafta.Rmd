...
knitr::opts_knit$set(root.dir = "/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Christine Chan/Prepost Triakfta/Data_Raw")
```
...
# Add BMI category
analytic_cm$bmicat = reg3_BMI$bmicat[match(analytic_cm$CFF.ID,reg3_BMI$CFF.ID)]
...
tab2b<- tableby(prepost~ppFVC + ppFEV1 + bmipct + Weight + BMI.Value, 
                data=bmidat4[bmidat4$CFF.ID %in% time[!is.na(time$prepost.x),]$CFF.ID,])
tab2b<- tableby(prepost~ppFVC + ppFEV1 + bmipct + Weight + BMI.Value, 
                data=bmidat4[bmidat4$CFF.ID %in% a$CFF.ID,])
...
tab3a <- tableby(time~ average_sensor
                + min_sensor
                + max_sensor
...
tab3b <- tableby(~age+bmipct + Weight + Sex + ppFEV1 + ppFVC  +
                  G.tube.in.past.12.months + Pancreatic.Status +
                  genoRisk + location+BMI.Value + age_category, 
                data=analytic_cm[analytic_cm$CFF.ID %in% cgm0_small$ID,])
summary(tab3a)
summary(tab3b)
summary(tab3c)
```
