...
gem <- gem %>%
  mutate(ETI = 0, pre_post = "Pre") %>%
  select(
    subject_id, date_of_visit, age_at_visit_1, gender, race_ethnicity,
    cf_genotype, cf_pancreatic, ETI, pre_post, average_bmi, fev1_percent,
...
# Merge, keep only pre/post
df <- full_join(df, gem) %>% filter(!is.na(pre_post))
