...
multi_pre_df = df |>
  ...
  mutate(
    ...
    time_diff_last_first = as.numeric(difftime(
      first(Date[pre_post == "Post"]),
      last(Date[pre_post == "Pre"]),
      units = "days"
    ))
  ) |>
  filter(n_pre > 1)
...
var_label(
  multi_pre_df$time_diff_last_first
) = "Days From Last Pre-ETI fsOGTT to First Post-ETI fsOGTT"
# Per Christine, exclude IA0077
multi_pre_df = multi_pre_df |> filter(study_id != "IA0077")
# Calculate days between pre- and post-ETI fsOGTTs
times = multi_pre_df |>
  group_by(study_id, pre_post) |>
  reframe(d = difftime(Date, lag(Date), units = "days"))
...
The median number of days between pre-ETI fsOGTTs was `r as.numeric(summary(times$d[times$pre_post=="Pre"])[3])` (IQR: `r as.numeric(summary(times$d[times$pre_post=="Pre"])[2])`, `r as.numeric(summary(times$d[times$pre_post=="Pre"])[5])`). The median number of days between post-ETI fsOGTTs was `r as.numeric(summary(times$d[times$pre_post=="Post"])[3])` (IQR: `r as.numeric(summary(times$d[times$pre_post=="Post"])[2])`, `r as.numeric(summary(times$d[times$pre_post=="Post"])[5])`).
...
# Model
m = lmer(
  matsuda ~ years_from_eti_start_model + slope_change + (1 | study_id),
  data = multi_pre_df
)
m |>
  tbl_regression(
