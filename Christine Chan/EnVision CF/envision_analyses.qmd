...
```{r}
# Fit
mod <- lmer(Insulin ~ Timepoint * Hypo70 + (1 | study_id),
  data = glmm_df
)
# EM means
means <- data.frame(emmeans(mod, c("Timepoint", "Hypo70"),
...
```{r}
# Fit
mod <- lmer(Insulin ~ Timepoint * Hypo60 + (1 | study_id),
  data = glmm_df
)
# EM means
means <- data.frame(emmeans(mod, c("Timepoint", "Hypo60"),
...
```{r}
# Fit
mod <- lmer(C.Peptide ~ Timepoint * Hypo70 + (1 | study_id),
  data = glmm_df
)
# EM means
means <- data.frame(emmeans(mod, c("Timepoint", "Hypo70"),
...
```{r}
# Fit
mod <- lmer(C.Peptide ~ Timepoint * Hypo60 + (1 | study_id),
  data = glmm_df
)
# EM means
means <- data.frame(emmeans(mod, c("Timepoint", "Hypo60"),
