...
# Select and merge
df <- df %>%
  select(study_id, redcap_event_name, num_hospitalizations, timepoint_120_min, homa_ir) %>%
  drop_na()
...
mod <- lm(df$timepoint_120_min ~ df$predicted)
summary(mod)
p <- plot(df$timepoint_120_min ~ df$predicted)
# Vs. homa-ir
mod <- lm(df$homa_ir ~ df$predicted)
summary(mod)
p <- plot(df$homa_ir ~ df$predicted)
# Vs. hospitalizations
...
# Vs. 2 hour
table(df$predicted, df$two_hour)
mod <- lm(df$Glucose_120 ~ df$predicted)
summary(mod)
