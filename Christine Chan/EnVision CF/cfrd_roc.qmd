...
df <- df %>% filter(!is.na(mean_sensor), !is.na(Diagnosis))
...
```{r results='asis'}
t1 <- tableby(
  Diagnosis ~ redcap_data_access_group + age_visit + height + weight +
    bmi_perc + sex + origin_race + ethnicity + pancreatic_status + CFTR + fev1 +
    fvc + a1c_result + corrector_yes_no,
  data = df
)
...
```{r results='asis'}
t2 <- tableby(
  Dysglycemia ~ redcap_data_access_group + age_visit + height + weight +
    bmi_perc + sex + origin_race + ethnicity + pancreatic_status + CFTR + fev1 +
    fvc + a1c_result + corrector_yes_no,
  data = df
)
...
```{r results='asis'}
t3 <- tableby(
  CFRD ~ redcap_data_access_group + age_visit + height + weight +
    bmi_perc + sex + origin_race + ethnicity + pancreatic_status + CFTR + fev1 +
    fvc + a1c_result + corrector_yes_no,
  data = df
)
