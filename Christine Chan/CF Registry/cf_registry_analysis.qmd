...
largest_gap = annual %>% 
  filter(eDWID %in% names(t)) %>% 
  select(eDWID,ReviewYear,cfrd_status_annual) %>%
  group_by(eDWID) %>%
  summarise(largest_gap = max(diff(ReviewYear)))
```

There were a total of `r nrow(largest_gap)` partipants who had a diagnosis of CFRD+ followed by at least one CFRD-, followed again by a CFRD+ diagnosis. For each person, we calculated the largest gap between CFRD+ diagnoses. 

```{r}
kable(table(largest_gap$largest_gap),
      col.names = c("Years between CFRD+","n"))
```
