...
encounter %>% group_by(eDWID,reviewyear) %>% 
  summarise(highest_2_hour_bg = suppressWarnings(max(twohour_bloodglucose,na.rm = T)),
            cfrd = max(cfrd_ogtt),
            cfrd_status = suppressWarnings(max(cfrd_status,na.rm = T)),
            .groups = "drop") %>%
  group_by(reviewyear) %>%
  summarise(`Prevalence by OGTT` = mean(as.numeric(cfrd)-1)*100,
            `Prevalence by CFFPR` = mean(cfrd_status == 3,na.rm = T)*100) %>%
  kable(.,digits = 3)
```
