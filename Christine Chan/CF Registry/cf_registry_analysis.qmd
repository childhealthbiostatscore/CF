...
annual <- read.csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Christine Chan/CF registry data/Data_Raw/CFF19_Annualized_Del1.csv",
  na.strings = c("", "N/A")
)
data_dict <- readxl::read_excel("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Christine Chan/CF registry data/Data_Raw/Copy of Codes for CFFPR_2018.xlsx")
...
# Get mutations
demogs <- read.csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Christine Chan/CF registry data/Data_Raw/CFF19_DemogCFDiag_Del1.csv")
...
log_reg <- function(year, missingness = F) {
  ...
  df <- annual %>%
    ...
    select(
      ...
      A_supplement_feeding, tube_feeding,
  ...
  vars <- colnames(df)[which(!colnames(df) %in%
    c(
      ...
      "A_feeding4", "A_FEV1", liver_fields
    ))]
