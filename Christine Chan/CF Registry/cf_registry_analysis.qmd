...
s = s %>% group_by(eDWID) %>% fill(cfrd_status_annual,.direction = "down")
s$cfrd_status_annual[is.na(s$cfrd_status_annual)] = "CFRD-"
# Remove records where start == end
s = s[s$end > s$start,]
# Survival fit
surv_obj = Surv(time = as.numeric(s$start),time2 = as.numeric(s$end),
                event = as.numeric(s$cfrd_status_annual)-1)
fit <- survfit(surv_obj ~ age_group,id = eDWID,data = s)
ggsurvplot(fit,risk.table = T,cumevents = T,
  cumcensor = T,
           fun = "event",xlim = c(2003, 2018))
```
