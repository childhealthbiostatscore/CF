...
forest_plot <- function(mod, time_varying = NULL, labels = labels) {
    coeff <- tbl_regression(cox_res,
        exponentiate = T,
        pvalue_fun = function(x) x
    ) %>%
        add_n(location = "level") %>%
        as_tibble()
    coeff_n <- tbl_regression(cox_res, exponentiate = T) %>%
        add_n(location = "label") %>%
        as_tibble()
    coeff$N2 <- coeff_n$`**N**`
    # Combine Ns
    coeff <- coeff %>% unite(N, `**N**`, N2, remove = F, na.rm = T)
    # Separate CI column
    coeff$hr <- as.numeric(coeff$`**HR**`)
    coeff$ll <- as.numeric(sapply(str_split(coeff$`**95% CI**`, ", "), "[", 1))
    coeff$ul <- as.numeric(sapply(str_split(coeff$`**95% CI**`, ", "), "[", 2))
    # Add N to names
    coeff$names <- paste0(coeff$`**Characteristic**`, " (N = ", coeff$N, ")")
    coeff$names <- ordered(coeff$names,
        levels = unique(coeff$names)
    )
    # P values
    ...
    # HR label
    coeff$hr_lab <- paste0(coeff$`**HR**`, " (", coeff$`**95% CI**`, ")")
    coeff$hr_lab[coeff$hr_lab == "NA (NA)"] <- NA
    coeff$hr_lab[coeff$type == "reference"] <- "Reference"
    # Plot
    p_left <- ggplot(coeff, aes(y = fct_rev(names))) +
        geom_text(aes(x = 0, label = names),
            hjust = 0,
            fontface = ifelse(coeff$type == "variable", "bold", "plain")
        ) +
        geom_text(aes(x = 1, label = hr_lab), hjust = 0) +
        theme_void() +
    ...
    p <- ggplot(coeff, aes(y = fct_rev(names))) +
        ...
        geom_stripes()
    # p values
    p_right <- ggplot(coeff, aes(y = fct_rev(names))) +
        geom_text(aes(x = 0, label = p),
            hjust = 0,
            fontface = ifelse(coeff$sig, "bold", "plain")
        ) +
        theme_void() +
        geom_stripes()
    # Layout
    p_left + p + p_right +
        plot_layout(widths = c(1, 2, 0.25), guides = "collect") &
        theme(plot.margin = margin(0, 0, 0, 0, "pt"))
    # Return each plot separately - will need to be combined outside of the function
    return(c(p_left, p, p_right))
}
```
