...
```{r}
tv_forest_plot <- function(mod, time_varying = NULL, time_variable = "tgroup",
                           labels = labels) {
  coeff <- tbl_regression(mod, pvalue_fun = function(x) x) %>%
    add_n(location = "level") %>%
    add_nevent(location = "level") %>%
    as_tibble()
  # If time-varying, format appropriately
  if (!is.null(time_varying)) {
    # Split table into time-varying and not
    w <- which(coeff$`**Characteristic**` == labels[time_varying])
    coeff <- coeff[w:nrow(coeff), ]
    t <- coeff %>% pivot_longer(`**Characteristic**`)
  }
  coeff_n <- tbl_regression(mod, exponentiate = T) %>%
    add_n(location = "label") %>%
    as_tibble()
  coeff$N2 <- coeff_n$`**N**`
  # Combine Ns
  coeff$N <- pmax(coeff$`**N**`, coeff$N2, na.rm = T)



  # Exponentiate
  coeff$hr <- exp(as.numeric(coeff$`**log(HR)**`))
  coeff$exp.ll <- exp(as.numeric(sapply(str_split(coeff$`**95% CI**`, ", "), "[", 1)))
  coeff$exp.ul <- exp(as.numeric(sapply(str_split(coeff$`**95% CI**`, ", "), "[", 2)))
  # Add N to names
  coeff$names <- paste0(coeff$`**Characteristic**`, " (N = ", coeff$N, ")")
  coeff$names <- ordered(coeff$names, levels = unique(coeff$names))
  # P values
  coeff$sig <- coeff$`**p-value**` < 0.05
  coeff$p <- style_pvalue(coeff$`**p-value**`)
  # Row types
  coeff$type <- "level"
  coeff$type[!is.na(coeff$N2)] <- "variable"
  coeff$type[!is.na(coeff$`**N**`) & is.na(coeff$hr)] <- "reference"
  # HR label
  coeff$hr_lab <- paste0(
    round(coeff$hr, 2), " (", round(coeff$exp.ll, 2), " - ",
    round(coeff$exp.ul, 2), ")"
  )
  coeff$hr_lab[coeff$hr_lab == "NA (NA - NA)"] <- NA
  coeff$hr_lab[coeff$type == "reference"] <- "Reference"
  #
}
```
...
plot_df <- tbl_regression(cox_res, exponentiate = T) %>%
  add_n(location = "level") %>%
  add_nevent(location = "level")
plot_df <- plot_df %>% as_tibble()
plot_df$expll <- as.numeric(sapply(strsplit(plot_df$`**95% CI**`, ", "), "[", 1))
plot_df$expul <- as.numeric(sapply(strsplit(plot_df$`**95% CI**`, ", "), "[", 2))
plot_df$hr <- as.numeric(plot_df$`**HR**`)
plot_df %>%
  forestplot(
    labeltext = c(`**Characteristic**`, `**N**`, `**Event N**`, `**HR**`),
    mean = "hr", lower = "expll", upper = "expul", xlog = T,
    is.summary = is.na(plot_df$`**N**`)
  ) %>%
  fp_add_header(
...
invisible(lapply(time_varying, function(v) {
  ...
  cat("\n")
  cat("\n")
  plot_df <- tbl_regression(cox_res_split) %>%
    add_n(location = "level") %>%
    add_nevent(location = "level") %>%
    as_tibble()
  plot_df$ll <- as.numeric(sapply(strsplit(plot_df$`**95% CI**`, ", "), "[", 1))
  plot_df$ul <- as.numeric(sapply(strsplit(plot_df$`**95% CI**`, ", "), "[", 2))
  plot_df$hr <- as.numeric(plot_df$`**log(HR)**`)
  plot_df %>%
    forestplot(
      labeltext = c(`**Characteristic**`, `**N**`, `**Event N**`, `**log(HR)**`),
      mean = "hr", lower = "ll", upper = "ul",
      is.summary = is.na(plot_df$`**N**`)
    ) %>%
    fp_add_header(
      `**Characteristic**` = c("", "Characteristic"),
