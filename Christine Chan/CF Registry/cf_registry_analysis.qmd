...
annual = annual %>% 
  group_by(eDWID) %>%
  mutate(age_2018 = as.numeric(names(sort(table(age_2018),decreasing = TRUE)[1])))
# Get mutations
...
annual$age_group <- cut(annual$Age_YrEnd, breaks = c(-Inf,10, 18, 30, Inf), right = F)
...
at_risk = at_risk %>% 
  full_join(.,annual %>% select(eDWID, ReviewYear,age_2018,A_hgba1c),
...
inc = inc %>% 
  group_by(eDWID) %>%
  mutate(age = age_2018[1] - (2018 - ReviewYear)) %>%
