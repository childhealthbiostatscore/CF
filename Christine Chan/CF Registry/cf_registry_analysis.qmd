...
```{r}
tv_forest_plot <- function(mod_coeff, time_variable = NULL, labels = labels) {
  coeff <- tbl_regression(mod, pvalue_fun = function(x) x) %>%
    add_n(location = "level") %>%
    add_nevent(location = "level") %>%
    as_tibble()
  # If time-varying, format appropriately
  if (length(grep("strata", coeff$`**Characteristic**`)) > 0) {
...
invisible(lapply(time_varying, function(v) {
  ...
  cox_res_split <- coxph(new_f, data = s_split, cluster = eDWID, contrasts = "contr.sum")
  ...
  cat("\n")
  cat("\n")
  emm <- data.frame(emmeans(cox_res_split_int, new_f_emm, nuisance = x[-which(x == v)]))
  plot_df <- data.frame(emm) %>% pivot_longer(cols = tgroup, value)
