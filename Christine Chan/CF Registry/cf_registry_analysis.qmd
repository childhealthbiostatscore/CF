...
gap_ids = unique(largest_gap$eDWID[largest_gap$largest_gap>=4])
...
# Survival analysis - second CFRD diagnosis (for those with $\geq$ 4 years between CFRD+ diagnoses)

```{r}
second_cfrd <- annual %>%
  filter(eDWID %in% gap_ids,cfrd_status_annual=="CFRD+") %>%
  group_by(eDWID) %>%
  summarise(second_cfrd = ReviewYear[2])
annual <- left_join(annual, second_cfrd, by = join_by(eDWID))
annual$first_cfrd = pmax(annual$first_cfrd,annual$second_cfrd)
```
