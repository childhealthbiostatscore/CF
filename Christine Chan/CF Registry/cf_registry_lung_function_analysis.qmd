...
# Calculate number of visits (just for table 1)
n_vis = annual |>
  group_by(eDWID) |>
  summarise(years_with_data = n())
cfrd_only <- left_join(cfrd_only, n_vis, by = join_by(eDWID))
```
...
Adjusted for age at diagnosis, sex, mutation class, and pancreatic enzyme use.
...
Adjusted for age at diagnosis, sex, mutation class, pancreatic enzyme use, and CFRD treatment.
