...
## NGT vs. (AGT + CFRD) and (NGT + AGT) vs. CFRD
...
ngt_agt <- roc_df |>
  mutate(
    ogtt = factor(
      `OGTT 2-hour`,
      levels = c("<140", "[140,200)", ">=200"),
      labels = c("NGT", "AGT + CFRD", "AGT + CFRD")
    )
  )
agt_cfrd <- roc_df |>
  mutate(
...
plot_df <- annual |>
  filter(
    ...
    Age_YrEnd >= 10,
    !cfrd_status_annual %in% c(4, 5)
  ) |>
  group_by(eDWID) |>
  mutate(
    Diagnosis = max(cfrd_status_annual),
    `Years to CFRD or Last Visit` = case_when(
      Diagnosis == 3 ~
        Age_YrEnd -
        first(Age_YrEnd[cfrd_status_annual == 3]),
      .default = Age_YrEnd - max(Age_YrEnd, na.rm = T)
    )
  )
plot_df$Diagnosis <- factor(
  plot_df$Diagnosis,
  levels = 1:3,
  labels = c("NGT", "AGT", "CFRD")
)
ggplot(
  plot_df,
  aes(
    y = A_FEV1_pct_predicted,
    x = `Years to CFRD or Last Visit`,
    color = Diagnosis,
    group = Diagnosis
  )
) +
