...
annual <- left_join(
  annual,
  demo %>% dplyr::select(eDWID, Race, Hispanicrace, Gender, F508, MutClass),
  by = join_by(eDWID)
)
...
# Plots
plot_df = lme_fit_a1c$data
plot_df$pred = predict(lme_fit_a1c)
p1 = ggplot(plot_df, aes(x = Age_YrEnd, y = pred)) +
  geom_point(alpha = 0.1, shape = ".") +
  geom_abline(intercept = 5.61, slope = 0.005895, color = "#3BA3BD") +
  ylab("HbA1c (%)") +
  xlab("Age (years)") +
  ylim(c(2.5, 15)) +
  ggtitle("Longitudinal Process") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))

fp_df = data.frame(s$`CoefTable-Event`) %>%
  rownames_to_column() %>%
  filter(!rowname %in% c("(Intercept)", "log(shape)")) %>%
  rename(estimate = "Value", term = "rowname") %>%
  mutate(
...
fp_df$term = factor(
  ...
  levels = c(
    ...
    "HispanicraceNo",
    "A_IsOnEnzymesNo",
    "NumPulmExacerbation1",
    "NumPulmExacerbation2",
    "NumPulmExacerbation>=3",
    "A_FEV1_pct_predicted",
    "bmi",
    "Assoct"
  ),
...
fp_df$term = as.character(fp_df$term)
fp_df$variable = fp_df$p.value <= 0.05
fp_df$estimate = exp(fp_df$estimate)
fp_df$conf.low = exp(fp_df$conf.low)
fp_df$conf.high = exp(fp_df$conf.high)
p2 = forestplot(
  df = fp_df,
  name = term,
  estimate = estimate,
  se = Std.Err,
  xlab = "Hazard ratio for CFRD",
  title = "Event Process",
  colour = variable
) +
  scale_color_manual(values = c("black", "#3BA3BD")) +
