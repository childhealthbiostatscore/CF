...
# Make survival datasets for registry and OGTT diagnoses
ogtt <- annual %>%
  select(
    eDWID, ReviewYear, Age_YrEnd, A_hgba1c, all_of(predictors), cfrd_ogtt
  ) %>%
  drop_na() %>%
  distinct()
surv_ogtt <- annual %>%
  ...
  select(eDWID, ReviewYear, Age_YrEnd, all_of(predictors), cfrd_ogtt) %>%
  drop_na() %>%
  distinct()
ogtt_include <- intersect(ogtt$eDWID, surv_ogtt$eDWID)
```
...
cox_f <- as.formula(paste0(
  "Surv(time = Age_YrEnd, event = cfrd_ogtt)~",
...
cox_fit <- coxph(cox_f,
  ...
  data = surv_ogtt[surv_ogtt$eDWID %in% ogtt_include, ]
)
lme_fit_a1c <- lme(lme_f,
  random = ~ Age_YrEnd | eDWID, na.action = na.omit,
  data = ogtt[ogtt$eDWID %in% ogtt_include, ]
)
# Combine
joint_fit <- jointModel(lme_fit_a1c, cox_fit, timeVar = "Age_YrEnd")
```
