...
bugs <- c(
  ...
  "A_pulmonarycomplications1", "A_mycocultureresults3", "A_cultureresults3"
)
# Yes/No variables
yn_vars <- c(
...
first_cfrd=head(first_cfrd,100)
...
first_cfrd$a1c_ogtt <- t
cfrd_only <- left_join(first_cfrd, annual |> remove_labels(),
  by = join_by(eDWID, first_cfrd == ReviewYear)
)
cfrd_only$a1c_ogtt <- factor(cfrd_only$a1c_ogtt,
  levels = c("both", "ogtt", "a1c", "neither"),
  labels = c("OGTT", "OGTT", "HbA1c Only", "Neither")
)
cfrd_only %>%
  select(
    a1c_ogtt, Age_YrEnd, A_weight, A_height, A_bmivalue, A_bmipercentile,
    A_FEV1_pct_predicted, A_FVC_pct_predicted, Race, Hispanicrace, MutClass,
    A_hgba1c, A_ogttfast_bloodglucose, A_twohour_bloodglucose, A_IsOnEnzymes,
    liver_disease, A_cultureresults3
  ) %>%
  tbl_summary(by = a1c_ogtt, missing_text = "Missing") %>%
  add_overall() %>%
