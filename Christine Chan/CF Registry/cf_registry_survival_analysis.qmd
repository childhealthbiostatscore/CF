...
ggsurvplot(km_fit,
  data = annual, risk.table = T, cumevents = T, fun = "event",
  xlab = "Days from 2003-01-01"
)
cum_inc <- tidy(km_fit)
cum_inc$time <- as.Date(cum_inc$time + og)
kable(cum_inc)
# By age group
km_fit <- survfit(
  Surv(as.numeric(tstart), as.numeric(tstop), as.numeric(CFRD) == 2,
...
km_table$time <- as.Date(km_table$time + og)
...
y <- "Surv(as.numeric(tstart), as.numeric(tstop), event)"
```
