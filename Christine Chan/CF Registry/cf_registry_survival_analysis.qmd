...
# Find each person's entry date
annual = annual %>% group_by(eDWID) %>%
  mutate(entry_date = min(ReviewYear))
...
labels <- list(
  ...
  eDWID = "ID", birth_year = "Birth year",
  tstart = "Interval start", tstop = "Interval stop",
  age_at_entry = "Age at entry", Age_YrEnd = "Age", bmi_perc = "BMI %ile"
)
```
...
```{r}
# Use flexsurv to fit a survival model that can be passed to rprev
llog <- flexsurv::flexsurvreg(Surv(tstop, as.numeric(CFRD)==2) ~ 1, 
                              data=prevdata, dist='llogis')
prev_llog <- prevalence(index='2004-01-01', 
                        num_years_to_estimate=1, 
                        data=annual, 
                        inc_formula = entrydate ~ 1,
                        surv_model=llog,
                        death_column = "first_cfrd",
                        incident_column = "first_cfrd",
                        N_boot = 100)
```
...
```{r}
# Whole cohort
km_fit <- survfit(Surv(tstart, tstop, as.numeric(CFRD)==2) ~ 1,
  data = annual, id = eDWID
)
ggsurvplot(km_fit,
  data = annual, risk.table = T, cumevents = T,
  fun = "event", xlim = c(2003, 2018), break.x.by = 1
)
kable(tidy(km_fit))
# By age group
km_fit <- survfit(Surv(tstart, tstop, as.numeric(CFRD)==2) ~ age_group,
  data = annual, id = eDWID
)
ggsurvplot(km_fit,
  data = annual, risk.table = T, cumevents = T,
...
y <- "Surv(tstart, tstop, as.numeric(CFRD)==2)"
...
```{r}
annual_split <- survSplit(Surv(tstart, tstop, as.numeric(CFRD)==2) ~ .,
  data = annual, cut = c(2008, 2013, 2018), episode = "tgroup"
)
...
invisible(lapply(time_varying, function(v) {
  ...
  cox_res_split_int <- coxph(new_f_int, data = annual_split, cluster = eDWID)
