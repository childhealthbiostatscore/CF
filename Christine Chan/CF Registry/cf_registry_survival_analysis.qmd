...
plot_df = plot_df %>%
  group_by(eDWID) %>%
  mutate(`Years to CFRD` = ReviewYear - first_cfrd)
ggplot(
  plot_df %>% filter(!is.na(a1c_ogtt)),
  aes(
...
screening <- annual %>%
  ...
  summarise(
    ...
    `Screened OGTT` = mean(
      !is.na(A_twohour_bloodglucose) |
        !is.na(A_ogttfast_bloodglucose)
    ),
    `Screened Either` = mean(
      !is.na(A_twohour_bloodglucose) |
        !is.na(A_ogttfast_bloodglucose) |
        !is.na(A_hgba1c)
    ),
...
screening_plot
ggsave(
  "./Dissemination/Figures/screening_rates.png",
  plot = screening_plot,
  width = 12,
  height = 9,
  units = "in"
...
i <- incidence %>%
  select(
    Year,
    `Total Incidence Age 10-18`,
    `Total Incidence Age 19-30`,
    `Total Incidence Age >= 31`
  ) %>%
  pivot_longer(
    `Total Incidence Age 10-18`:`Total Incidence Age >= 31`,
    names_to = "Age Group"
  )
...
fig1 <- fig1a +
  ...
  plot_annotation(
    title = "Figure 1: Trends in Screening, Incidence, and Prevalence of CFRD from 2003-2018, by age group "
  )
fig1
ggsave(
  "./Dissemination/Figures/fig1.png",
  plot = fig1,
  width = 12,
  height = 3,
  units = "in",
...
fig1 <- fig1a +
  ...
  plot_layout(guides = "collect") +
  plot_annotation(
    title = "US Cystic Fibrosis Foundation Patient Registry Trends in Screening, Incidence, and Prevalence of Cystic Fibrosis Related Diabetes from 2003-2018"
  )
fig1
ggsave(
  "./Dissemination/Figures/graphical_abstract.png",
  plot = fig1,
  width = 13,
  height = 3,
  units = "in",
...
f_vx770 <- update(
  f,
  . ~ . + A_Vx770 - A_corticosteroids1 - NumPulmExacerbation - age_group
)
vx770_cox_res <- coxph(
  f_vx770,
...
comb_plot
ggsave(
  "./Dissemination/Figures/vx_combined_forest.png",
  plot = comb_plot,
  width = 12,
  height = 9,
  units = "in"
...
comb_plot
ggsave(
  "./Dissemination/Figures/time_combined_forest.png",
  plot = comb_plot,
  width = 12,
  height = 9,
  units = "in",
