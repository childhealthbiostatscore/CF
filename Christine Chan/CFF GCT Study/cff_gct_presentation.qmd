...
- Impaired fasting glucose (IFG):
  - FPG 100–125 mg/dL with OGTT-2hPG <140 mg/dL; 
- Impaired glucose tolerance (IGT): was defined as 
  - FPG <126 mg/dL
  - OGTT-2hPG 140–199 mg/dL
- CFRD:
  - FPG ≥126 mg/L and/or OGTT-2hPG ≥ 200 mg/dL.
...
## Sensitivity of the OR rule

\begin{aligned}
Se(X_1 \vee X_2) &= 1 - P((X_1 = 0, X_2 = 0) \mid D = 1) \\
                 &= 1 - P(X_2 = 0 \mid X_1 = 0, D = 1)(1 − Se(X_1)) \\
                 &=Se(X1) + (1 − Se(X1))Se(X2) − \tau_{+}

\end{aligned}

where $\tau_{+} = \rho_{+}[Se(X_1)Se(X_2)(1 − Se(X_1))(1 − Se(X_2))]^{0.5}$

## Specificity of the OR rule

\begin{aligned}
Sp(X_1 \vee X_2) &= P((X_1 = 0, X2 = 0) \mid D = 0) \\
                 &= P(X2 = 0 \mid X1 = 0, D = 0)Sp(X1) \\
                 &=Sp(X1)Sp(X2) + \tau_{-}

\end{aligned}

where $\tau_{-} = \rho_{+}[Sp(X_1)Sp(X_2)(1 − Sp(X_1))(1 − Sp(X_2))]^{0.5}$
...
- Combined sensitivity increases when the two tests are **negatively correlated** given disease **positive**.
- Combined specificity increases when the two tests are **positively correlated** given disease **negative**.
- The sensitivity of the combined test is greater than or equal to that of either test administered on its own. 
- The specificity is less than or equal to an individual test.
...
- Missclassification of reference standard tests.
...
## Analyzing test sequences with an imperfect standard
...
- One approach is to model the conditional dependence between tests using the covariance between tests, treating disease status as a latent variable.
  - Unfortunately, the fully-parameterised model is not identifiable.
