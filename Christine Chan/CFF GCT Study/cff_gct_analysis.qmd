...
# Dotplot
dotplot <- ggplot(df, aes(x = Diagnosis, y = `GCT 60 min plasma`)) +
  geom_point() +
  ylab("GCTpl (mg/dL)") +
  theme_bw()
dotplot
...
plot_df <- df %>%
  pivot_longer(c(`OGTT 0 min`, `OGTT 60 min`, `OGTT 120 min`, `GCT 60 min plasma`),
    names_to = "OGTT Timepoint", values_to = "Glucose"
  )
plot_df$`OGTT Timepoint` <- factor(plot_df$`OGTT Timepoint`,
  levels = c("OGTT 0 min", "OGTT 60 min", "OGTT 120 min", "GCT 60 min plasma"),
  labels = c("Fasting", "OGTT-1hG", "OGTT-2hG", "GCTpl")
)
box_p <- ggplot(
  plot_df,
  aes(x = `OGTT Timepoint`, y = Glucose, fill = `OGTT Timepoint`)
...
# GCTpl, RCG, HbA1c, OGTT1hr
plot_df <- df %>%
  pivot_longer(
    c(
      `GCT 60 min plasma`, `GCT 0 min capillary`, HbA1c,
      `OGTT 60 min`
    ),
...
box_p1 <- ggplot(
  ...
  aes(x = Diabetes, y = Value, fill = Diabetes)
) +
  geom_boxplot() +
  geom_hline(yintercept = 146.5, linetype = "dashed") +
  ylim(c(50, 300)) +
  ylab("mg/dL") +
  ggtitle("GCTpl") +
...
box_p2 <- ggplot(
  ...
  geom_boxplot() +
  geom_hline(yintercept = 98.5, linetype = "dashed") +
  ylim(c(50, 300)) +
  ggtitle("RCG") +
  theme_bw(base_size = 15) +
  theme(
...
box_p3 <- ggplot(
  ...
  geom_boxplot() +
  geom_hline(yintercept = 5.45, linetype = "dashed") +
  ylab("%") +
  ggtitle("HbA1c") +
  theme_bw(base_size = 15) +
  theme(
...
box_p4 <- ggplot(
  plot_df[plot_df$Measure == "OGTT 60 min", ],
  aes(x = Diabetes, y = Value, fill = Diabetes)
) +
  geom_boxplot() +
  geom_hline(yintercept = 228.5, linetype = "dashed") +
  ylim(c(50, 300)) +
  ggtitle("OGTT 60 min") +
  theme_bw(base_size = 15) +
  theme(
...
box_p <- box_p1 + box_p2 + box_p3 + box_p4
ggsave("./Christine Chan/CFF GCT Study/Dissemination/Figures/boxplot2.png",
  box_p,
  device = "png"
)
# Dotplot
box_p1 <- ggplot(
  ...
  geom_jitter() +
  geom_hline(yintercept = 146.5, linetype = "dashed", size = 1) +
  ylim(c(50, 300)) +
  ylab("Glucose (mg/dL)") +
  ggtitle("GCTpl") +
  theme_bw(base_size = 15) +
...
box_p2 <- ggplot(
  ...
  geom_jitter() +
  geom_hline(yintercept = 98.5, linetype = "dashed", size = 1) +
  ylim(c(50, 300)) +
  ggtitle("RCG") +
  theme_bw(base_size = 15) +
  scale_colour_manual(values = RColorBrewer::brewer.pal(3, "Set2")) +
...
box_p4 <- ggplot(
  ...
  geom_jitter() +
  geom_hline(yintercept = 228.5, linetype = "dashed", size = 1) +
  ylim(c(50, 300)) +
  ggtitle("OGTT 1hPG") +
  theme_bw(base_size = 15) +
  scale_colour_manual(values = RColorBrewer::brewer.pal(3, "Set2")) +
...
box_p <- box_p1 + box_p2 + box_p4 + box_p3 + plot_layout(ncol = 4)
...
### `GCT 0 min capillary` and HbA1c

```{r}
f <- lrm(CFRD ~ `GCT 0 min capillary` + HbA1c,
  data = df
)
ddist <- datadist(df)
options(datadist = "ddist")
nom <- nomogram(f, fun = plogis, funlabel = "Risk of CFRD", lp = F)
plot(nom)
```

Here’s how to use a nomogram. Pick a point on the GCT 0 min capillary and HbA1c scales and get the associated points for each. Add the points together to get the associated probability based on the Total Points line.

For example:

- GCT 0 min capillary = 130 (40 points)
- HbA1c = 6.6 (10 points)

Add up the points:

40 + 10 = 50 Total Points

50 Total Points lines up at about 0.08, so this person has approximately an 8% chance of CFRD. The process is the same for the nomograms below.

### `GCT 0 min capillary`, HbA1c, and `GCT 60 min plasma`

```{r}
f <- lrm(CFRD ~ `GCT 0 min capillary` + HbA1c + `GCT 60 min plasma`,
  data = df
)
ddist <- datadist(df)
options(datadist = "ddist")
...
### `GCT 0 min capillary`, and `GCT 60 min plasma`

```{r}
f <- lrm(CFRD ~ `GCT 0 min capillary` + `GCT 60 min plasma`,
  data = df
)
ddist <- datadist(df)
options(datadist = "ddist")
...
```{r}
tree <- rpart(CFRD ~ `GCT 0 min capillary` + HbA1c + `GCT 60 min plasma`,
  data = df
)
rpart.plot(tree, type = 0)
```

We just don't have enough data to do this properly for CFRD.

## Predicting dysglycemia

```{r}
tree <- rpart(Dysglycemia ~ `GCT 0 min capillary` + `GCT 60 min plasma`, data = df)
rpart.plot(tree, type = 0)
```
