...
data$clinic_cx_no_growth.factor <- factor(data$clinic_cx_no_growth.factor,
  levels = 0:2,
  labels = c("No Growth", "Any Growth", "Any Growth")
)
...
# Times from home collection
data$days_to_home_received <-
  as.numeric(difftime(as.Date(data$home_cx_date_received), as.Date(data$home_cx_date),
    units = "days"
  ))
data$days_to_home_received[data$subject_id == "009-49"] <- NA
label(data$days_to_home_received) <-
  "Days from Home Collection to Home Collection Received"
data$days_to_home_collection <-
  as.numeric(difftime(as.Date(data$home_cx_date), as.Date(data$clinic_date),
    units = "days"
  ))
label(data$days_to_home_collection) <-
  "Days from Clinic Collection to Home Collection"
```
...
data %>%
  select(
    ...
    mutation2.factor, Genotype, rural_location.factor, cftr_modulator.factor,
    fev1_pp, fvc_pp, days_to_home_received, days_to_home_collection,
    cx_past_results___1.factor:cx_past_results___11.factor
  ) %>%
  mutate(
  ...
  tbl_summary(
    ...
    type = list(c(days_to_home_received, days_to_home_collection) ~
      "continuous2"),
    statistic = list(
      all_continuous() ~ "{mean} ({sd})",
      c(days_to_home_received, days_to_home_collection) ~
        "{median} [{p25}, {p75}]"
    )
  ) %>%
  as_kable_extra()
  ...
  bar_plot_df <- d %>%
    ...
    mutate(
      "Immediate vs. Delayed" = case_when(
        ...
        Immediate == "Any Growth" & Delayed == "Any Growth" ~ "Any Growth on Both",
        Immediate == "Absent" & Delayed == "Absent" ~ "Both Absent",
        Immediate == "No Growth" & Delayed == "No Growth" ~ "No Growth on Either",
        .default = "Disagreed"
      ),
      "Immediate vs. Home" = case_when(
        Immediate == "Present" & Home == "Present" ~ "Both Present",
        Immediate == "Any Growth" & Home == "Any Growth" ~ "Any Growth on Both",
        Immediate == "Absent" & Home == "Absent" ~ "Both Absent",
        Immediate == "No Growth" & Home == "No Growth" ~ "No Growth on Either",
        .default = "Disagreed"
      ),
      "Delayed vs. Home" = case_when(
        Delayed == "Present" & Home == "Present" ~ "Both Present",
        Delayed == "Any Growth" & Home == "Any Growth" ~ "Any Growth on Both",
        Delayed == "Absent" & Home == "Absent" ~ "Both Absent",
        Delayed == "No Growth" & Home == "No Growth" ~ "No Growth on Either",
        .default = "Disagreed"
      ),
    ) %>%
...
# Why didn't people prefer home collection? (for reviewer response)
no_home_collection <- survey_data %>%
  filter(q6.factor == "No") %>%
  select(participant_id, q6, q6b___1:q6b___4, q6b_spec)
colnames(no_home_collection) <- label(no_home_collection)
kable(no_home_collection)
```
...
p3 <- gglikert(as_tibble(df3), add_totals = F) +
  scale_fill_manual(values = c("#7895B2", "#AEBDCA", "#E8DFCA")) +
