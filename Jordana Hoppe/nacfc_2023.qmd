...
```{r}
fleiss = function(bug,bug_name){
  fk = kappam.fleiss(df[,c(bug,paste0(bug,".1"),paste0(bug,".2"))],detail = T)
  t = as.data.frame.matrix(fk$detail)
...
```{r results='asis'}
fleiss(bug = "Methicillin.sensitive.Staph.aureus.",bug_name = "MSSA")
```

```{r results='asis'}
fleiss(bug = "Methicillin.resistant.Staph.aureus.",bug_name = "MRSA")
```

```{r results='asis'}
fleiss("A..xylosoxidans.", bug_name = "A. xylosoxidans")
```

```{r results='asis'}
fleiss("Achromobacter.not.xylosoxidans.",bug_name = "Achromobacter (not xylosoxidans)")
```

```{r results='asis'}
fleiss("Acinetobacter.", bug_name = "Acinetobacter")
```

```{r results='asis'}
fleiss("H..influenzae.", bug_name = "H. influenzae")
```

```{r results='asis'}
fleiss("P..aeruginosa.", bug_name = "P. aeruginosa")
```

```{r results='asis'}
fleiss("P..aeruginosa..mucoid..",bug_name = "P. aeruginosa (mucoid)")
```

```{r results='asis'}
fleiss("Pseudomonas.species..not.aeruginosa.",bug_name = "Pseudomonas (not aeruginosa)")
```

```{r results='asis'}
fleiss("B..cepacia.complex.", bug_name = "B. cepacia complex")
```

```{r results='asis'}
fleiss("Stenotrophomonas.", bug_name = "Stenotrophomonas")
```

```{r results='asis'}
fleiss("Aspergillus.",bug_name = "Aspergillus")
```

```{r results='asis'}
fleiss("Penicillium.species.",bug_name = "Penicillium species")
```

```{r results='asis'}
fleiss("Sceosporium.species.",bug_name = "Sceosporium species")
```

```{r results='asis'}
fleiss("M..avium.complex.",bug_name = "M. avium complex")
```

```{r results='asis'}
fleiss("M..abscessus.", bug_name = "M. abscessus")
```
