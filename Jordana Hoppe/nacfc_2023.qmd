...
library(irr)
library(gtsummary)
df <- read.csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Jordana Hoppe/Home Collection/Data_Clean/clinical_2023-06-20.csv", na.strings = "")
surveys <- read.csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Jordana Hoppe/Home Collection/Data_Clean/surveys_2023-06-20.csv", na.strings = "")
# Combined genotype
...
bug_names=c("Methicillin sensitive Staph aureus", 
...
f = df %>% select(Subject.ID,contains (bugs)) %>% 
  mutate(across(-Subject.ID,
                ~as.numeric(factor(.x,levels=c("Unchecked","Checked")))-1)) %>%
  rowwise() %>%
  mutate(`Clinic % Positive` = mean(c_across(all_of(bugs)))*100,
