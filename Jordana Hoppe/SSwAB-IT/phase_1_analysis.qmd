...
home_dir <- switch(
  Sys.info()["sysname"],
  "Darwin" = "/Users/tim/Library/CloudStorage/OneDrive-TheUniversityofColoradoDenver/Vigers/CF/Jordana Hoppe/SSwAB-IT",
...
db_demographics <- left_join(
  db_demographics,
  db_clinic_obtained_values %>%
    select(record_id, cos_collection_date, ppfev1),
  by = join_by(record_id)
)
db_demographics$age_at_cos <-
  as.numeric(difftime(
    db_demographics$cos_collection_date,
    db_demographics$dob,
    units = "days"
  )) /
  365.25
# 1-5, 6-11, 12-17 and 18+ years
db_demographics$age_group <- cut(
  db_demographics$age_at_cos,
  c(-Inf, 6, 12, 18, Inf),
  right = F
)
...
db_demographics$Race <- factor(
  db_demographics$Race,
  levels = c(
    "White",
    "Other",
    "Multiple Races",
...
db_demographics$Ethnicity <- factor(
  db_demographics$Ethnicity,
...
db_medications <- left_join(
  db_medications,
  db_demographics %>% select(record_id, group),
  by = join_by(record_id)
)
db_clinic_obtained_values <- left_join(
  db_clinic_obtained_values,
  db_demographics %>% select(record_id, group),
  by = join_by(record_id)
)
...
df[, paste0(bugs, "_cos")] <- lapply(
  df[, paste0(bugs, "_cos")],
  factor,
  levels = c("No", "Yes"),
  labels = c("Negative", "Positive")
)
df[, paste0(bugs, "_sos")] <- lapply(
  df[, paste0(bugs, "_sos")],
  factor,
  levels = c("No", "Yes"),
  labels = c("Negative", "Positive")
...
df <- left_join(
  df,
  db_demographics %>% select(record_id, group, age_group),
  by = join_by(record_id)
)
...
db_medications %>%
  ...
  select(
    -inhaled_colisitin_status,
    -inhaled_vancomycin_status,
    -other_inhaled_status,
    -please_specify_mucolytic
  ) %>%
...
db_feasibility %>%
  select(
    previous_home_sample:if_no_why_not___4,
    -barrier_other,
    -barriers___5,
    -if_no_why_not___4
  ) %>%
...
agreement <- function(bug, data = df) {
  ...
  pabak <- epi.kappa(table(d$COS, d$SOS))
  # Print
  cat("\n")
  cat("\n")
  cat("####", "2x2 Table")
  ...
  cat("\n")
  d1 = d %>%
    tbl_cross(percent = "cell", digits = c(0, 1)) %>%
    print()
  cat("\n")
  cat("\n")
  ...
  d %>%
    tbl_summary(
      by = COS,
      percent = "cell",
      digits = list("SOS" ~ c(0, 1))
    ) %>%
    add_ci(
      pattern = "{stat} [{ci}]",
      method = list(all_categorical() ~ "wald"),
      style_fun = all_categorical() ~ label_style_number(
        scale = 100,
        digits = 1
      )
    ) %>%
  ...
  cat("\n")
  cat(
    "Overall accuracy (both negative/negative and positive/positive) was ",
    scales::percent(c$overall["Accuracy"], accuracy = 0.1),
    " (95% CI: ",
  ...
  cat(
    "Kappa was ",
    round(pabak$kappa$est, 3),
    " (95% CI: ",
    round(pabak$kappa$lower, 3),
    " - ",
    round(pabak$kappa$upper, 3),
    ")",
    ".",
    sep = ""
  )
  ...
  cat("\n")
  cat(
    "Prevalence and Bias Adjusted Kappa (PABAK) was ",
    round(pabak$pabak$est, 3),
    " (95% CI: ",
    round(pabak$pabak$lower, 3),
    " - ",
    round(pabak$pabak$upper, 3),
    ")",
    ".",
    sep = ""
  )
...
agreement_test <- function(bug, group = "group", data = df) {
  ...
  d$detailed_agree <- factor(
    d$detailed_agree,
    levels = c(
      "COS Negative/SOS Negative",
      "COS Negative/SOS Positive",
      "COS Positive/SOS Negative",
      "COS Positive/SOS Positive"
    ),
  ...
  d %>%
    ...
    tbl_summary(
      ...
      digits = list(c("agree", "detailed_agree") ~ c(0, 1))
    ) %>%
    add_overall() %>%
    add_p(
      test = list(agree ~ "chisq.test", detailed_agree ~ "fisher.test"),
      test.args = all_tests("fisher.test") ~ list(workspace = 2e9)
    ) %>%
    separate_p_footnotes() %>%
    modify_footnote(everything() ~ NA) %>%
    print()
...
a2 <-
  ggplot(
    as.data.frame(table(d_overall$COS, d_overall$SOS)),
    aes(Var1, Var2)
  ) +
...
a1 <- plot(
  euler(d_overall == "Positive"),
  quantities = list(cex = 3),
  main = "Any CF Pathogen"
)
b1 <- plot(
  euler(d_MSSA == "Positive"),
  quantities = list(cex = 3),
  main = "MSSA"
)
p2 <- ggarrange(a1, b1, nrow = 1, ncol = 2) +
  bgcolor("White")
ggsave(
  "./Dissemination/Figures/euler_bw.png",
  plot = p2,
  width = 9,
  height = 6,
  units = "in"
...
a1 <- plot(
  venn(d_overall == "Positive"),
  quantities = list(cex = 3),
  main = "Any CF Pathogen"
)
b1 <- plot(
  venn(d_MSSA == "Positive"),
  quantities = list(cex = 3),
  main = "MSSA"
)
p3 <- ggarrange(a1, b1, nrow = 1, ncol = 2) +
  bgcolor("White")
ggsave(
  "./Dissemination/Figures/venn_bw.png",
  plot = p3,
  width = 9,
  height = 6,
  units = "in"
