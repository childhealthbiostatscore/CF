...
otu_from_explicet=function(data){
  ...
  sum=data$Bacteria+data$Unclassified
  data$Bacteria=NULL
  data$Unclassified=NULL
  data$root=data$root-sum
  return(data)}
...
###PcOA Plot
PCOA_plot<-function(data,root,color,group=NULL,group_name=NULL,paired){
...
prevalence<-function(data,group=NULL){
  if(is.null(group)==F){apply(data,2,function(x){tapply(as.numeric(x),group,function(y){sum(ifelse(y==0,0,1))/(dim(data)[1]/2)})})}
  else{apply(data,2,function(x){sum(ifelse(as.numeric(x)==0,0,1))/dim(data)[1]})}}
...
wilcox_table<-function(data,root,RA_cutoff,Prevalence_cutoff,group,paired){
  ...
  rel_otu=cbind.data.frame(apply(data,2,
                                 function(x){
                                   x=as.numeric(as.character(x))/as.numeric(as.character(root))}))
  
  Taxa_keep=names(na.omit(apply(rel_otu,2,function(x){ifelse(max(x)>RA_cutoff,x,NA)})))
  ...
  wilx_rel_otu=apply(wilx_rel_otu,2,as.numeric)
  ...
  if(length(keep_wilx)==0){
    return("No Significant Comparisons")
  }
  else{
    ...
    names=unlist(name_split(names(keep_wilx)))
    #Table One
    table=cbind.data.frame(names,
...
#####Stacked Bar Chart
stacked_barchart<-function(data,y,x,fill){
  ggplot(data=data, 
       aes(y=y, 
           x=x, 
           fill=factor(fill))) + 
...
spearman_correlations<-function(data,root,correlate){
  rel_otu=cbind.data.frame(apply(data,2,function(x){
        x=as.numeric(as.character(x))/as.numeric(as.character(root))}))
  
  save=do.call(rbind,apply(rel_otu[,grep("Bacteria",colnames(rel_otu))],2,function(x){
    `P-value`=cor.test(x=x,y=correlate,method="spearman")$p.value
    Rho=cor.test(x=x,y=correlate,method="spearman")$estimate
    df=cbind.data.frame(Rho,`P-value`)
    return(df)}))
  save=save[save$`P-value`<.05,]
  row.names(save)=name_split(row.names(save))
  save=round(save,3)
  save$Genera=row.names(save)
  row.names(save)<-NULL
  save=na.omit(save)
