...
library(Hmisc)
# Import REDCap data
redcap <- read.csv("/Users/timvigers/Dropbox/Work/Vigers/CF/Scott Sagel/PEx in CF/Data_Raw/ExacerbationsInCF222_DATA_2023-10-04_1459.csv", na.strings = "")
...
supplement <- readxl::read_excel("/Users/timvigers/Dropbox/Work/Vigers/CF/Scott Sagel/PEx in CF/Data_Raw/ExacerbationsInCF222-Admissions and Inpatient Meds All Fields 2023 09 29.xlsx")
supplement <- supplement %>% select(
  sid, redcap_repeat_instance,
  admit_year:age_at_admit, height, fev1_admit, fev1_discharge
)
...
df <- df %>% select(all_of(colnames(demo)), everything())
...
labels <- list(
  ...
  "admit_year" = "Year",
  "days_admit" = "# Days in Admit",
  "age_at_admit" = "Age",
  "height" = "Height",
  "ppfev1_admit" = "FEV1 % Predicted at Admit",
  "ppfev1_discharge" = "FEV1 % Predicted at Discharge"
)
label(df) <- labels[colnames(df)]
# Save
save(df, labels,
  file = "/Users/timvigers/Dropbox/Work/Vigers/CF/Scott Sagel/PEx in CF/Data_Cleaned/analysis_dataset.RData"
)
