...
library(ggpubr)
library(rspiro)
knitr::opts_knit$set(root.dir = "/home/tim/OneDrive/Vigers/CF/Scott Sagel/PEx in CF")
# Colors for plots
yellow <- "#f8b80a"
red <- "#e30022"
purple <- "#7b2670"
...
labels <- list(
  ...
  "Baseline FEV1pp (2022)" = "ppfev1_baseline",
  "Admission FEV1pp (2022)" = "ppfev1_admit",
  "Discharge FEV1pp (2022)" = "ppfev1_discharge",
...
df <- df %>%
  select(
    ...
    ppfev1_baseline, ppfev1_admit, ppfev1_discharge, days_admit,
    fungal_test, fungi___1:fungi___5,
    ntm_test, ntm___1:ntm___3,
    viral_test, viruses___1:viruses___5,
    admit_cx_results___1:admit_cx_results___9,
    contains("steroids")
  ) %>%
  filter(!is.na(year))
# Write for Scott and Yaron to check
write.csv(df,file = "./Data_Clean/analysis_dataset.csv",row.names = F,na = "")
```
...
```{r}
#| label: tbl-table-1
#| tbl-cap: Participant characteristics at admission
df %>%
...
df %>%
  ...
  select(year, redcap_data_access_group, viruses___1:viruses___5) %>%
  tbl_summary(by = year, missing_text = "Missing", label = labels) %>%
  add_overall() %>%
  add_p() %>%
  separate_p_footnotes()
...
df %>%
  filter(viral_test == "Yes") %>%
  select(year, cftr_mod, viruses___1:viruses___5) %>%
  tbl_strata(
    strata = year,
    .tbl_fun =
      ~ .x %>%
...
df %>%
  filter(viral_test == "Yes") %>%
  select(year, cftr_mod_spec, viruses___1:viruses___5) %>%
  tbl_strata(
    strata = year,
    .tbl_fun =
      ~ .x %>%
...
df %>%
  filter(viral_test == "Yes") %>%
  select(year, Trikafta, viruses___1:viruses___5) %>%
  tbl_strata(
    strata = year,
    .tbl_fun =
      ~ .x %>%
...
Hospitalizations within a person were considered independent in the table below.

```{r}
#| label: tbl-fev1-change-eti
#| tbl-cap: Change in FEV1pp by ETI status
df %>%
  mutate(ppfev1_change = ppfev1_discharge - ppfev1_admit) %>%
  filter(year == 2022) %>%
