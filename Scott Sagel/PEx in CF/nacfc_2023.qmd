...
df %>%
  group_by(redcap_data_access_group, admit_year) %>%
  summarise(n = n(), .groups = "drop") %>%
  ggplot(aes(x = redcap_data_access_group, y = n, fill = factor(admit_year))) +
...
df %>%
  ...
  ggplot(aes(x = redcap_data_access_group, y = perc, fill = factor(admit_year))) +
  geom_bar(position = "dodge", stat = "identity") +
  xlab("Site") +
...
df %>%
  ...
  ggplot(aes(x = redcap_data_access_group, y = age_at_admit, 
...
# Sex
df %>%
  group_by(redcap_data_access_group, admit_year, sid) %>%
  ...
  geom_text(aes(label = paste(round(perc, 1), "%")),
    position = position_stack(vjust = 0.5), size = 2
  ) +
  ylab("%") +
  facet_grid(~redcap_data_access_group) +
  theme_bw() +
...
```{r}
#| warning: false
df %>% group_by(redcap_data_access_group,admit_year,sid) %>%
  summarise(across(ppfev1_admit:ppfev1_baseline,~mean(.x,na.rm=T)),
            .groups = "drop") %>%
  rename(Admission = ppfev1_admit,Discharge = ppfev1_discharge,
  ...
  ggplot(aes(x = name, y = value, 
             fill = factor(admit_year))) +
  geom_boxplot() + ylab("FEV1 % predicted") +
  facet_wrap(~redcap_data_access_group)+
  theme_bw() +
