...
# Import
chco <- read_excel("/Users/timvigers/Documents/Work/Vigers/CF/Scott Sagel/PEx in CF/Data_Clean/CHCO.xlsx", na = c("", "NULL"))
cnh <- read_excel("/Users/timvigers/Documents/Work/Vigers/CF/Scott Sagel/PEx in CF/Data_Clean/CNH.xlsx")
# Format CHCO columns
...
cnh <- cnh %>%
  rename(
    `# Days in Admit` = `Total hospitalization days`,
...
bugs <- c("Staph aureus", "MRSA", "Haemophilus influenzae", "P. aeruginosa non-mucoid", "P. aeruginosa mucoid", "Pseudomonas not aeruginosa", "Burkholderia", "burkhodescription_other", "A. xylosoxidans", "Achromobacter not xylosoxidans", "S. maltophilia", "Inquilinus limosus", "Pandoraea species", "Other bacteria", "Aspergillus species", "Candida species", "Pencillium species", "Scedosporium species", "Mycobacterium avium complex", "Other mycobacterium", "P. aeruginosa")
df[, bugs] <- lapply(df[, bugs], function(c) {
  c[is.na(c)] <- "No"
  c <- factor(c, levels = c("No", TRUE), labels = c("No", "Yes"), ordered = T)
})
df$`S. maltophilia`[df$Site == "CNH"] <- NA
# Write cleaned data
write.csv(df, file = "/Users/timvigers/Documents/Work/Vigers/CF/Scott Sagel/PEx in CF/Data_Clean/pex_merged.csv", row.names = F, na = "")
# Make summary table
summ <- df %>%
  group_by(`Unique number`, Site, Year) %>%
  mutate(`Change in FEV1` = `FEV1 % Predicted at Discharge` - `FEV1 % Predicted at Admit`) %>%
  summarise(
    `# of Hospitalizations per Person` = n(),
...
# Import REDCap data from Eli
data = read.csv("~/Documents/Work/Vigers/CF/Scott Sagel/PEx in CF/Data_Raw/ExacerbationsInCF222_DATA_2023-10-04_1459.csv",na.strings = "")
# Demographics
demo = data %>% filter(redcap_event_name == "demographics_arm_1")
```
