...
cfqr_parent_6_13 <- df %>%
  select(
    sid, redcap_event_name:redcap_repeat_instance,
    cfq_r_p_1:cfq_r_p_44
  ) %>%
  mutate(across(cfq_r_p_1:cfq_r_p_44, ~ as.numeric(.x))) %>%
...
check <- left_join(
  ...
  cfqr_parent_6_13 %>%
    select(sid, redcap_event_name:redcap_repeat_instance, physical:weight),
  join_by(
    sid, redcap_event_name, redcap_repeat_instrument,
    redcap_repeat_instance
  )
)
write.csv(check, file = "./Data_Clean/cfqr_parent_6_13_check.csv", na = "", row.names = F)

cfqr_children_6_11 <- list(
...
cfqr_children_6_11 <-
  setNames(names(cfqr_children_6_11), cfqr_children_6_11)
cfqr_teens_adults <- list(
...
cfqr_teens_adults <-
  setNames(names(cfqr_teens_adults), cfqr_teens_adults)
```
