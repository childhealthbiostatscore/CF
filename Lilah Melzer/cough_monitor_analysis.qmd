...
#| fig-cap: Coughs by nights from start
#| warning: false
ggplot(agg_counts[agg_counts$sid == 2 & !is.na(agg_counts$Coughs), ], aes(
...
```{r}
#| label: fig-coughs-by-night-3
#| fig-cap: Coughs by nights from start
#| warning: false
ggplot(agg_counts[agg_counts$sid == 3 & agg_counts$Night %in% c(1:7, 96:102), ], aes(
  x = Night, y = Coughs, group = factor(sid), color = Sick
)) +
  geom_point() +
  geom_line() +
