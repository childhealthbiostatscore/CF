...
# Combine
cf_surveys <- full_join(cfqr_parent_6_13, cfqr_children_6_11)
cf_surveys <- full_join(cf_surveys, cfqr_children_12_13)
cf_surveys <- full_join(cf_surveys, cfqr_teens_adults)
cf_surveys$sid <- as.numeric(cf_surveys$sid)
# Add to counts for plotting
agg_counts <- full_join(agg_counts, cf_surveys, by = join_by(sid, Date))
agg_counts <- full_join(agg_counts, hc_surveys, by = join_by(sid, Date, Survey))
```
