...
library(corrplot)
library(patchwork)
knitr::opts_knit$set(root.dir = "/Users/timvigers/Library/CloudStorage/OneDrive-TheUniversityofColoradoDenver/Vigers/CF/Lilah Melzer/Lilah Cough Monitor")
```
...
cfqr_parent_6_13 <- df %>%
  select(
    sid, cfqr_caregivers_613_timestamp, cfq_r_p_1:cfq_r_p_44
  ) %>%
  mutate(
  ...
  mutate(
    physical =
      ifelse(
        sum(is.na(c(phys1, phys2, phys3, phys4, phys5, phys14, phys15, phys16))) <= 4,
  ...
  select(sid, Date, physical:weight)
...
cfqr_children_6_11 <- df %>%
  select(
    sid, cfqr_children_ages_611_timestamp, cfq_r_y_1:cfq_r_y_35
  ) %>%
  mutate(
  ...
  mutate(
    ...
    emotion =
      ifelse(
        sum(is.na(c(emot7, emot8, emot9, emot10, emot11, emot12, emot13, emot14))) <= 4,
        (mean(c(emot7, emot8, emot9, emot10, emot11, emot12, emot13, emot14)) - 1)
        / 3 * 100, NA
      ),
    social =
      ifelse(
        sum(is.na(c(social20, social21, social22, social23, social24, social25, social26))) <= 3,
        (mean(c(social20, social21, social22, social23, social24, social25, social26)) - 1)
        / 3 * 100, NA
      ),
    eat =
      ifelse(
  ...
  select(sid, Date, physical:digest)
# Teen/Adult version
...
cfqr_teens_adults <- df %>%
  select(
    sid, cfqr_adolescents_and_adults_14_years_timestamp, cfq_r_a_1:cfq_r_a_50
  ) %>%
  mutate(
  ...
  mutate(
    ...
    social =
      ifelse(
        sum(is.na(c(social22, social23, social27, social28, social29, social30))) <= 3,
        (mean(c(social22, social23, social27, social28, social29, social30)) - 1)
        / 3 * 100, NA
      ),
    body =
      ifelse(
...
write.csv(df %>%
  select(
    sid, cfqr_caregivers_613_timestamp, cfq_r_p_1:cfq_r_p_44
  ), file = "~/cfqr_parent_6_13.csv", na = "", row.names = F)
write.csv(df %>%
  select(
    sid, cfqr_children_ages_611_timestamp, cfq_r_y_1:cfq_r_y_35
  ), file = "~/cfqr_children_6_11.csv", na = "", row.names = F)
write.csv(df %>%
  select(
    sid, cfqr_adolescents_and_adults_14_years_timestamp, cfq_r_a_1:cfq_r_a_50
  ), file = "~/cfqr_teens_adults.csv", na = "", row.names = F)
# Add to counts for plotting
...
agg_counts <- left_join(agg_counts, cfqr_parent_6_13)
agg_counts <- left_join(agg_counts, cfqr_children_6_11)
agg_counts <- left_join(agg_counts, cfqr_teens_adults)
```
...
invisible(lapply(unique(agg_counts$sid), function(s) {
  ...
  p2 <- ggplot(d[!is.na(d$respirat), ], aes(
    x = Night, y = respirat, group = factor(sid), color = Sick
  )) +
    geom_point() +
    geom_line() +
    theme_bw() +
  ...
  cat("\n")
  cat("\n")
  print(p + p2 + plot_layout(guides = "collect"))
  cat("\n")
  cat("\n")
}))
```
