...
demo <- df %>%
  ...
  select(
    ...
    cftr_mod, cftr_mod_type, mucolytic:antih, pulm_ex, contains("micro_type___")
  )
...
t1 <- tableby(
  Diagnosis ~ nih_sex + age_at_consent + height_perc + weight_perc +
    ...
    total_sleep + pulm_ex + micro_type___1 + micro_type___2 + micro_type___3 +
    micro_type___4,
...
t2 <- tableby(
  Diagnosis ~ `Nights` + `Nights With Data` + `Prop. Nights With Data` +
    `Prop. Nights With Cough` + `Median Coughs per Night`,
