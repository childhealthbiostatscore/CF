...
So, instead of the model approach I did a more simple correlation between median coughs per night and median respiratory score.
...
caregivers <-
  full_join(agg_counts, cfqr_parent_6_13, join_by(sid, Date)) %>%
  select(sid, Sick, Night, Coughs, respirat) %>%
  filter(!if_all(Coughs:respirat, ~ is.na(.)))
caregivers_med <- caregivers %>%
  group_by(sid) %>%
  summarise(
    `Median Coughs` = median(Coughs, na.rm = T),
    `Median Resp. Score` = median(respirat, na.rm = T)
  ) %>%
  drop_na()
M <- cor.test(caregivers_med$`Median Coughs`,
  caregivers_med$`Median Resp. Score`,
  method = "spearman"
)
ggplot(caregivers_med, aes(
  y = `Median Coughs`, x = `Median Resp. Score`,
  colour = factor(sid)
)) +
  geom_point() +
  theme_bw()
...
Spearman's correlation between median coughs and median respiratory symptom score was `r round(as.numeric(M$estimate),3)` (p = `r format.pval(M$p.value,digits = 3,eps=0.001)`).
...
#| message: false
#| warning: false
invisible(lapply(unique(caregivers_med$sid), function(s) {
  d <- caregivers[caregivers$sid == s, ]
  d <- d %>% pivot_longer(c(Coughs, respirat))
  p <- ggplot(d, aes(x = Night, y = value, color = Sick)) +
    geom_point() +
    geom_line() +
    theme_bw() +
    scale_color_manual(values = c("#377EB8", "#E41A1C")) +
    facet_wrap(~name, ncol = 1) +
    ggtitle(paste0("Participant ", s))
  cat("\n")
  cat("\n")
  print(p)
  cat("\n")
...
children_12_13 <-
  full_join(agg_counts, cfqr_children_12_13, join_by(sid, Date)) %>%
  select(sid, Sick, Night, Coughs, respirat) %>%
  filter(!if_all(Coughs:respirat, ~ is.na(.)))
children_12_13_med <- children_12_13 %>%
  group_by(sid) %>%
  summarise(
    `Median Coughs` = median(Coughs, na.rm = T),
    `Median Resp. Score` = median(respirat, na.rm = T)
  ...
  drop_na()
M <- cor.test(children_12_13_med$`Median Coughs`,
  children_12_13_med$`Median Resp. Score`,
  method = "spearman"
)
ggplot(children_12_13_med, aes(
  y = `Median Coughs`, x = `Median Resp. Score`,
  colour = factor(sid)
)) +
  geom_point() +
...
#| message: false
#| warning: false
invisible(lapply(unique(children_12_13_med$sid), function(s) {
  d <- children_12_13[children_12_13$sid == s, ]
  d <- d %>% pivot_longer(c(Coughs, respirat))
  p <- ggplot(d, aes(x = Night, y = value, color = Sick)) +
    geom_point() +
    geom_line() +
...
### CFQ-R Adolescents and Adults 14 Years

#### Median scores

```{r}
#| warning: false
cfqr_teens_adults$sid <- as.numeric(cfqr_teens_adults$sid)
teens_adults <-
  full_join(agg_counts, cfqr_teens_adults, join_by(sid, Date)) %>%
  select(sid, Sick, Night, Coughs, respirat) %>%
  filter(!if_all(Coughs:respirat, ~ is.na(.)))
teens_adults_med <- teens_adults %>%
  group_by(sid) %>%
  summarise(
    `Median Coughs` = median(Coughs, na.rm = T),
    `Median Resp. Score` = median(respirat, na.rm = T)
  ...
  drop_na()
M <- cor.test(teens_adults_med$`Median Coughs`,
  teens_adults_med$`Median Resp. Score`,
  method = "spearman"
)
ggplot(teens_adults_med, aes(
  y = `Median Coughs`, x = `Median Resp. Score`,
  colour = factor(sid)
)) +
  geom_point() +
...
#| message: false
#| warning: false
invisible(lapply(unique(teens_adults_med$sid), function(s) {
  d <- teens_adults[teens_adults$sid == s, ]
  d <- d %>% pivot_longer(c(Coughs, respirat))
  p <- ggplot(d, aes(x = Night, y = value, color = Sick)) +
    geom_point() +
    geom_line() +
...
hc_surveys$sid <- as.numeric(hc_surveys$sid)
hcs <-
  full_join(agg_counts, hc_surveys, join_by(sid, Date)) %>%
  select(sid, Sick, Night, Coughs, q1:q5) %>%
  filter(!if_all(q1:q5, ~ is.na(.)))
hcs_med <- hcs %>%
  group_by(sid) %>%
  summarise(
    `Median Coughs` = median(Coughs, na.rm = T),
    `Median Resp. Score` = median(as.numeric(q1), na.rm = T)
  ) %>%
  drop_na()
M <- cor.test(hcs_med$`Median Coughs`,
  hcs_med$`Median Resp. Score`,
  method = "spearman"
)
ggplot(hcs_med, aes(
  y = `Median Coughs`, x = `Median Resp. Score`,
