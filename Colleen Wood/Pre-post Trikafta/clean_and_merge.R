...
# CHCO
chco_a1c <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/chco_glycemic.csv", show_col_types = F)
chco_a1c$Date <- parse_date_time(chco_a1c$Date, orders = "mdy")
chco_bmi <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/chco_bmi_pft.csv", show_col_types = F)
...
chco_reg <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/chco_registry.csv", show_col_types = F)
...
keep <- chco_reg %>%
  ...
  pull(`CFF ID`)
exclude <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/chco_insulin.csv", show_col_types = F)
exclude <- exclude %>%
...
# Montana
mon_bmi <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/montana_bmi_pft.csv", show_col_types = F)
mon_bmi$Date <- parse_date_time(mon_bmi$Date, orders = "mdy")
mon_a1c <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/montana_glycemic_registry.csv", show_col_types = F)
...
# Add demographics
mon_demo <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/montana_demo.csv", show_col_types = F)
mon_demo <- mon_demo %>% select(`CFF ID`, Race, Sex)
mon <- left_join(mon, mon_demo)
# Add genotypes
mon_geno <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/montana_genotypes.csv", show_col_types = F)
mon <- left_join(mon, mon_geno)
# Add pancreatic and g tube status
mon_pan <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/montana_pancreatic_insufficient.csv", show_col_types = F)
mon_pan <- mon_pan %>%
  select(-Date) %>%
  distinct()
mon_pan$`Pancreatic Status` <- "Insufficient"
mon_gtube <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/montana_gtubes.csv", show_col_types = F)
mon_gtube <- mon_gtube %>%
  select(-Feeding, -Date) %>%
  distinct()
...
# UM
um_a1c <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/um_a1cs.csv", show_col_types = F)
um_a1c$Age <- round(um_a1c$Age / 365.25, 1)
um_bmi <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/um_bmi.csv", show_col_types = F)
um_bmi$Age <- round(um_bmi$Age / 365.25, 1)
um_ogtt <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/um_ogtt.csv", show_col_types = F)
um_ogtt$Age <- round(um_ogtt$Age / 365.25, 1)
um_pft <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/um_pfts.csv", show_col_types = F)
um_pft$Age <- round(um_pft$Age / 365.25, 1)
um_race <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/um_race.csv", show_col_types = F)
um_demo <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/um_demo.csv", show_col_types = F)
keep <- um_demo %>%
  filter(`CFRD Status` == "CFRD with or without fasting hyperglycemia") %>%
  pull(`CFF ID`)
um_demo <- um_demo %>%
...
# UW
uw_bmi <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_bmi.csv", show_col_types = F)
uw_bmi$Date <- parse_date_time(uw_bmi$Date, orders = "mdy")
uw_glyc <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_glycemic.csv", show_col_types = F)
uw_glyc$Date <- parse_date_time(uw_glyc$Date, orders = "mdy")
uw_pft <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_pft.csv", show_col_types = F)
uw_pft$Date <- parse_date_time(uw_pft$Date, orders = "mdy")
uw_demo <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_demo.csv", show_col_types = F)
uw_demo <- uw_demo %>% select(`CFF ID`, Race, Sex)
# Get CFRD status and start date
uw_start <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_mods.csv", show_col_types = F)
...
uw_cfrd <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_cfrd.csv", show_col_types = F)
uw_cfrd <- uw_cfrd %>%
  filter(`CFRD Status` == "CFRD with or without fasting hyperglycemia") %>%
  select(`CFF ID`) %>%
  distinct() %>%
...
# Add genotypes
uw_geno <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_genotypes.csv", show_col_types = F)
# Pancreatic status
uw_pan <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_pancreatic.csv", show_col_types = F)
uw_pan <- uw_pan %>%
  select(-Date) %>%
  distinct() %>%
  mutate(`Pancreatic Status` = "Insufficient")
# G tubes
uw_gtube <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_gtubes.csv", show_col_types = F)
uw_gtube <- uw_gtube %>%
  select(`CFF ID`) %>%
  distinct() %>%
...
chco_ins <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/chco_insulin.csv", show_col_types = F)
...
chco_ins <- chco_ins %>%
  ...
  filter(!is.na(`CFF ID`))
mon_ins <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/montana_insulin.csv", show_col_types = F)
mon_ins <- mon_ins %>%
  rename(
    TDD = `Total Daily Dose of insulin (units)`,
    `Long acting insulin dose` = `Long acting or total basal daily dose (units)`
  ...
  select(`CFF ID`, `Pre or Post`, TDD, `Long acting insulin dose`)
uw_ins <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_insulin.csv", show_col_types = F)
uw_ins <- uw_ins %>%
  rename(
    TDD = `Total Daily Dose of insulin (units)`,
    `Long acting insulin dose` = `Long acting or total basal daily dose (units)`
...
# CGM data
cgm_vars <- c(
  "average_sensor", "min_sensor", "max_sensor",
...
day_cgm <- c(
  ...
  "percent_time_under_60_day", "percent_time_under_70_day", "daytime_sd"
)
night_cgm <- c(
  ...
  "percent_time_under_60_night", "percent_time_under_70_night", "nighttime_sd"
)
# Add CGM information
# CHCO
chco_cgm <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/chco_cgm.csv", show_col_types = F)
chco_cgm$`Pre or Post` <- factor(sapply(str_split(chco_cgm$subject_id, "_"), "[", 2))
levels(chco_cgm$`Pre or Post`) <- c("Post", "Pre", "Pre")
...
# Montana
mon_cgm <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/montana_cgm.csv", show_col_types = F)
...
# UW
uw_cgm <- read_csv("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/uw_cgm.csv", show_col_types = F)
...
df <- left_join(df, cgm)
save(df, cgm_vars, day_cgm, night_cgm, file = "/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood/Prepost Triakfta/Data_Cleaned/analysis_dataset.rdata")
