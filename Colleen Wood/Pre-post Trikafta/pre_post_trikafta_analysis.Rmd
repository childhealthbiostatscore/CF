...
chco$`BMI Percentile` = round(sds(chco$BMI,age = chco$age_for_bmi,item = "bmi",type = "perc",
...
mon$`BMI Percentile` = round(sds(mon$BMI,age = mon$age_for_bmi,item = "bmi",type = "perc",
...
um = um %>% filter(`CFF ID` %in% keep,!is.na(Time),abs(Time) <= 365) %>% 
  rename(Site = "site") %>% arrange(`CFF ID`,Time)
...
um$`BMI Percentile` = round(sds(um$BMI,age = um$age_for_bmi,item = "bmi",type = "perc",
...
uw$`BMI Percentile 2` = round(sds(uw$BMI,age = uw$age_for_bmi,item = "bmi",type = "perc",
...
df = df %>% arrange(`CFF ID`,Time) %>% group_by(`CFF ID`,`Pre or Post`) %>%
  summarise(`A1c Values` = sum(!is.na(HbA1c)),
...
```{r results='asis'}
# Table
t1 = tableby(~Age+`BMI Percentile`+Weight+Sex+ppFEV1+ppFVC+`G-tube in past 12 months`+
               `Pancreatic Status`+Site+Genotype+`Age Group`,
