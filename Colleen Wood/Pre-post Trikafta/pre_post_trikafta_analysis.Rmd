...
most_recent = df %>% group_by(CFF.ID,Site,Before_After) %>%
  ...
  summarise(Start = first(na.omit(Start)),
            Age = paste0(Date[which(Age == first(na.omit(Age)))],": ",round(first(na.omit(Age)),1)),
...
most_recent$Start = as.character(most_recent$Start)
...
# Write 
write.csv(most_recent,file = "./Colleen Wood/Prepost Triakfta/Data_Cleaned/pre_post.csv",
          na = "",row.names = F)
```
