...
most_recent = df %>% group_by(CFF.ID,Site,Before_After) %>%
  ...
  arrange(time_new) %>% 
  summarise(Age = first(na.omit(Age)),BMI.Percentile = first(na.omit(BMI.Percentile)),
            Weight = first(na.omit(Weight)),Sex = first(na.omit(Sex)),
            T1D.Duration = first(na.omit(T1D.Duration)),
            ...
            BMIz = first(na.omit(BMIz)),Weightz = first(na.omit(Weightz)),
            FEV1 = first(na.omit(FEV1)),FVC = first(na.omit(FVC)),
            HbA1c = first(na.omit(HbA1c)),.groups = "keep") %>%
