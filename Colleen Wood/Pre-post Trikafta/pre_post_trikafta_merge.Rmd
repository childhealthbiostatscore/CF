...
if(Sys.info()["sysname"] == "Windows"){
  home_dir = "S:/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood"
} else if (Sys.info()["sysname"] == "Linux"){
  home_dir = "~/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood"
} else if (Sys.info()["sysname"] == "Darwin"){
  home_dir = "/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Vigers/CF/Colleen Wood"
}
...
chco_bmi_pft = read_csv("./Prepost Triakfta/Data_Cleaned/chco_bmi_pft.csv")
...
chco_cgm = read_csv("./Prepost Triakfta/Data_Cleaned/chco_cgm.csv")
...
chco_glycemic = read_csv("./Prepost Triakfta/Data_Cleaned/chco_glycemic.csv")
chco_glycemic$Date = mdy(chco_glycemic$Date)
# chco registry 108 obs
# CFF ID and MRN, start date
chco_registry = read_csv("./Prepost Triakfta/Data_Cleaned/chco_registry.csv")
###################################
...
montana_bmi_pft <- read_csv("./Prepost Triakfta/Data_Cleaned/montana_bmi_pft.csv")

# cgm 16 obs
montana_cgm <- read_csv("./Prepost Triakfta/Data_Cleaned/montana_cgm.csv")
...
montana_demo <- read_csv("./Prepost Triakfta/Data_Cleaned/montana_demo.csv")
montana_genotypes <- read_csv("./Prepost Triakfta/Data_Cleaned/montana_genotypes.csv")
montana_gtubes <- read_csv("./Prepost Triakfta/Data_Cleaned/montana_gtubes.csv") # what are the dates here
montana_pancreatic <- read_csv("./Prepost Triakfta/Data_Cleaned/montana_pancreatic.csv") # what is this? just dates and cffid
# ask tim:what do we need to do with gtubes and pancreatic dates? dont look like chco data

# glycemic 279
montana_glycemic_registry <- read_csv("./Prepost Triakfta/Data_Cleaned/montana_glycemic_registry.csv")
montana_glycemic_registry$Date = mdy(montana_glycemic_registry$Date)
# looks identical
###################################

###### IMPORT UW DATA #############
uw_registry <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_registry.csv") #cffid and start
uw_bmi <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_bmi.csv") # date, cffid, height, bmi (height bmi need rename)
uw_pft <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_pft.csv") # datee, cffid, fev, fvc need renaming
uw_cgm <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_cgm.csv") # same code as montana
uw_demo <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_demo.csv") 
uw_genotypes <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_registry.csv") # no genotype data here?
uw_glycemic <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_glycemic.csv") # looks good, same as chco
uw_gtubes <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_gtubes.csv") # rename date of encounter to date
uw_pancreatic <- read_csv("./Prepost Triakfta/Data_Cleaned/uw_pancreatic.csv") # same as montana, no pancreatic data though

###################################

###### IMPORT UM DATA ########
#um_all <- read_csv("./Prepost Triakfta/Data_Cleaned/um_all.csv")
###################################
...
write.csv(analysis_data, file = "./Prepost Triakfta/Data_Cleaned/analysis_data.csv")
```