...
feature_scores <- scores %>%
  ...
  summarise(Majority = factor(sum(value == "Present", na.rm = T),
...
feature_scores %>%
  pivot_wider(names_from = Feature, values_from = Score) %>%
  mutate(`Total Score` = rowSums(.[3:12])) %>%
  select(-scan_id) %>%
  tbl_summary(
    ...
    type = list(
      c(
        ...
        "Tree In Bud OR Nodule", "Total Score"
      ) ~ "continuous2"
    ),
...
lobe_scores %>%
  pivot_wider(names_from = Location, values_from = Score) %>%
  mutate(`Total Score` = rowSums(.[3:8])) %>%
  select(-scan_id) %>%
  tbl_summary(
    by = `NTM-PD`, missing_text = "Missing",
    type = list(
      c("RUL", "RML", "RLL", "LUL", "Ling", "LLL", "Total Score") ~ "continuous2"
    ),
    statistic = all_continuous() ~ c("{median} ({p25}, {p75})", "{min}, {max}")
  ) %>%
  add_p(
...
# Add gold standards
scores <- rbind(scores, data_gs[data_gs$scorer != "GS", ])
# Get a total by scorer
plot_df <- scores %>%
  group_by(scan_id, scorer) %>%
  summarise(sum = sum(value == "Present"), .groups = "drop") %>%
  pivot_wider(names_from = "scorer", values_from = sum)
# Plots
ab <- plot_df %>%
  ggplot(., aes(x = A, y = B)) +
  geom_point() +
