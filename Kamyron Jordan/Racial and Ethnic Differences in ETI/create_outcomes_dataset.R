...
# Import
encounter <- read.csv(
  "./Kamyron Jordan/Racial and Ethnic Differences in ETI/Data_Raw/DataDelivery_20240125/CFF22_encountersMerged_Del1.csv",
  na.strings = ""
)
annual <- read.csv(
  "./Kamyron Jordan/Racial and Ethnic Differences in ETI/Data_Raw/DataDelivery_20240125/CFF22_Annualized_Del1.csv",
  na.strings = ""
)
demo <- read.csv(
  "./Kamyron Jordan/Racial and Ethnic Differences in ETI/Data_Raw/DataDelivery_20240125/CFF22_DemogCFDiag_Del1.csv",
  na.strings = ""
)
hospitalizations <- read.csv(
  "./Kamyron Jordan/Racial and Ethnic Differences in ETI/Data_Raw/DataDelivery_20240125/CFF22_CareEpisodes_Del1.csv",
  na.strings = ""
)
# List of outcomes
...
encounter$encounterlocation <- factor(
  encounter$encounterlocation,
  levels = 1:6,
  labels = c("Clinic", "Hospital", "Home IV", "Other", "5", "6")
)
encounter$bacterialculturedone[is.na(encounter$bacterialculturedone)] <- 0
encounter$bacterialculturedone <- factor(
  encounter$bacterialculturedone,
  levels = 0:1,
  labels = c("No", "Yes")
)
encounter$cultureresults <- factor(
  encounter$cultureresults,
  levels = 1:3,
...
# Age group
encounter$age_group <- cut(
  encounter$encounterage,
  breaks = c(6, 12, 18, 30, Inf),
  right = F
)
...
no_post <- t1_participant$eDWID[
  t1_participant$`Number of Encounters Post-ETI` == 0
]
...
# Make labels
tots1 <- paste0(
  format(n_people_1, big.mark = ","),
  " People\n",
  format(n_enc_1, big.mark = ","),
  " Encounters"
)
no_date <- paste0(
  "No ETI Date:\n",
  format(n_people_no_eti_date, big.mark = ","),
  " People\n",
  format(length(no_eti_date), big.mark = ","),
  " Encounters"
)
tots2 <- paste0(
  format(n_people_2, big.mark = ","),
  " People\n",
  format(n_enc_2, , big.mark = ","),
  " Encounters"
)
hosps <- paste0(
  "Care Episodes:\n",
  format(n_people_hosp, big.mark = ","),
  " People\n",
  format(length(hosp), big.mark = ","),
  " Encounters"
)
tots3 <- paste0(
  format(n_people_3, big.mark = ","),
  " People\n",
  format(n_enc_3, big.mark = ","),
  " Encounters"
)
pregs <- paste0(
  "Pregnancies:\n",
  format(n_people_preg, big.mark = ","),
  " People\n",
  format(length(preg), big.mark = ","),
  " Encounters"
)
tots4 <- paste0(
  format(n_people_4, big.mark = ","),
  " People\n",
  format(n_enc_4, big.mark = ","),
  " Encounters"
)
earlies <- paste0(
  "Visits > ",
  format(365.25 * 3, big.mark = ","),
  " Days BEFORE ETI:\n",
  format(n_people_early, big.mark = ","),
  " People\n",
  format(length(early), big.mark = ","),
  " Encounters"
)
lates <- paste0(
  "Visits > ",
  format(365.25 * 2, big.mark = ","),
  " Days AFTER ETI:\n",
  format(n_people_late, big.mark = ","),
  " People\n",
  format(length(late), big.mark = ","),
  " Encounters"
)
tots5 <- paste0(
  format(n_people_5, big.mark = ","),
  " People\n",
  format(n_enc_5, big.mark = ","),
  " Encounters"
)
no_posts <- paste0(
  "No Post-ETI Visits:\n",
  format(length(no_post), big.mark = ","),
  " People\n",
  format(length(no_post_visits), big.mark = ","),
  " Encounters (Pre-ETI)"
)
tots6 <- paste0(
  format(n_people_6, big.mark = ","),
  " People\n",
  format(n_enc_6, big.mark = ","),
  " Encounters"
)
fev1s <- paste0(
  format(length(low_fev1), big.mark = ","),
  " FEV1 < ",
  format(lower, big.mark = ","),
  "\n",
  format(length(high_fev1), big.mark = ","),
  " FEV1 > ",
  format(upper, big.mark = ",")
)
fvcs <- paste0(
  format(length(low_fvc), big.mark = ","),
  " FVC < ",
  format(lower, big.mark = ","),
  "\n",
  format(length(high_fvc), big.mark = ","),
  " FVC > ",
  format(upper, big.mark = ",")
)
# Assemble the chart!
