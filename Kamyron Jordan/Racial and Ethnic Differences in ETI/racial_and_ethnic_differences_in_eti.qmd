...
For the following analysis, the start time ($t=0$) was the approximate date that each person became eligible for ETI. Because we don't have exact dates of birth, I calculated an approximate DOB based on age and encounter dates.
...
surv_dat$Vx445comb <- as.numeric(factor(surv_dat$Vx445comb, levels = c("No", "Yes")))
# Filter for KM
km_dat <- surv_dat %>%
  group_by(eDWID) %>%
  slice_max(stop)
kmfit <- survfit(Surv(stop, Vx445comb) ~ race, data = km_dat[km_dat$race != "Asian", ])
autoplot(kmfit) + theme_bw()
# Cox PH model
fit <- coxph(Surv(start, stop, Vx445comb) ~ race, id = eDWID, data = surv_dat)
```
...
gt(means) %>% fmt_number(emmean:last_col(), decimals = 3)
```
...
```{r}
ggplot(left_join(annual, means, by = join_by(race, ReviewYear))) +
  geom_line(mapping = aes(x = ReviewYear, y = mean_ppFEV, group = eDWID), alpha = 0.005, linewidth = 0.5) +
  geom_point(mapping = aes(x = ReviewYear, y = emmean, color = race)) +
  geom_line(mapping = aes(x = ReviewYear, y = emmean, group = race, color = race)) +
  geom_errorbar(mapping = aes(x = ReviewYear, ymin = emmean - SE, ymax = emmean + SE, color = race), width = .2) +
  xlab("Year") +
...
#| eval: false
t <- annual %>%
  select(eDWID, ReviewYear, mean_ppFEV) %>%
  na.omit(.)
model <- list(
  mean_ppFEV ~ 0 + ReviewYear,
  ~ 0 + ReviewYear
)
