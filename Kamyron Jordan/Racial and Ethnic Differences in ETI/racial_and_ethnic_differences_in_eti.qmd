...
```{r}
table1(~ race | Hispanicrace, data=demo)
```
...
```{r}
table1(~race+Hispanicrace|factor(ReviewYear),data = annual)
```
...
```{r}
table1(~Gender+WasGenotyped+F508_num|race,data=by_year)
```
...
```{r}
encounter %>%
  mutate(eti_elig = factor(eti_elig, ordered = T)) %>%
  group_by(eDWID, reviewyear) %>%
  summarise(
    eti_elig = max(eti_elig), race = first(na.omit(race)),
    .groups = "drop"
  ) %>%
  select(-eDWID) %>%
  rename(any_of(tidy_labels)) %>%
  tbl_summary(by = `Race/Ethnicity`, missing = "Missing") %>%
        bold_labels() %>%
        as_kable_extra()
  tbl_strata(
    strata = Year,
