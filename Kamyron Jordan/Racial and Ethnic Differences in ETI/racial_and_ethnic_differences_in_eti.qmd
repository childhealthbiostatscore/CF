...
```{r results='asis'}
invisible(lapply(c("2019", "2020", "2021", "2022"), function(n) {
  cat("\n")
  cat("###", n)
  cat("\n")
  cat("\n")
...
surv_dat <- encounter %>%
  ...
  select(
    ...
    mother_education, encounterdate, encounterage, eti_elig_date, Vx445comb,
    start, stop
  ) %>%
...
```{r}
# Quantiles
tbl_survfit(fit, probs = c(0.25, 0.5, 0.75))
```
...
```{r}
cox_res <- coxph(surv_f, data = surv_dat, cluster = eDWID)
cox_res %>%
  tbl_regression() %>%
...
#### Adjusted for insurance status, family income, and education

```{r}
new_surv_f <- update(surv_f, . ~ . + Insurance + family_income +
  patient_education + father_education + mother_education)
...
#### Adjusted for insurance status only
...
State special needs program, e.g., BCMH, CCS, CRS, GHPP, etc. = Other
...
The most basic model we can fit to these data uses the average values as the outcome, with terms for review year, race, and their interaction and a random intercept for each patient. This model is adjusted for age, private vs. other insurance, and parental education.
