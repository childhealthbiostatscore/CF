...
change_mod <- lmer(
  gli_fev1_ppred_rn ~ Years * Race + slope_change_years * Race + age_group +
    Insurance + prior_modulator + isOnEnzymes + (1 | eDWID),
  data = encounter
)
# Model plot
...
# Linear hypotheses
white_vs_black_pre <- glht(
  change_mod,
  "Years + `Years:RaceBlack or African American` = 1.195465"
)
white_vs_black_pre <- tidy(summary(white_vs_black_pre))
white_vs_black_pre$contrast <- "Black vs. White Pre-ETI Slope"
white_vs_black_pre %>%
  gt() %>%
  fmt_number(decimals = 3)
white_vs_black_post <- glht(
  change_mod,
  "Years + `Years:RaceBlack or African American` + slope_change_years + `RaceBlack or African American:slope_change_years` = 2.738885"
)
white_vs_black_post <- tidy(summary(white_vs_black_post))
white_vs_black_post$contrast <- "Black vs. White Post-ETI Slope"
white_vs_black_post %>%
  gt() %>%
  fmt_number(decimals = 3)

white_vs_hispanic_pre <- glht(
  change_mod,
  "Years + `Years:RaceHispanic or Latino` = 1.19547"
)
white_vs_hispanic_pre <- tidy(summary(white_vs_hispanic_pre))
white_vs_hispanic_pre$contrast <- "Hispanic vs. White Pre-ETI Slope"
white_vs_hispanic_pre %>%
  gt() %>%
  fmt_number(decimals = 3)
white_vs_hispanic_post <- glht(
  change_mod,
  "Years + `Years:RaceHispanic or Latino` + slope_change_years + `RaceHispanic or Latino:slope_change_years` = 2.738885"
)
white_vs_hispanic_post <- tidy(summary(white_vs_hispanic_post))
white_vs_hispanic_post$contrast <- "Hispanic vs. White Post-ETI Slope"
white_vs_hispanic_post %>%
  gt() %>%
  fmt_number(decimals = 3)

white_vs_other_pre <- glht(
  change_mod,
  "Years + `Years:RaceOther` = 1.19547"
)
white_vs_other_pre <- tidy(summary(white_vs_other_pre))
white_vs_other_pre$contrast <- "Other vs. White Pre-ETI Slope"
white_vs_other_pre %>%
  gt() %>%
  fmt_number(decimals = 3)
white_vs_other_post <- glht(
  change_mod,
  "Years + `Years:RaceOther` + slope_change_years + `RaceOther:slope_change_years` = 2.738885"
)
white_vs_other_post <- tidy(summary(white_vs_other_post))
white_vs_other_post$contrast <- "Other vs. White Post-ETI Slope"
white_vs_other_post %>%
  gt() %>%
  fmt_number(decimals = 3)
```
...
change_mod <- lmer(
  GLI_FEV1_pct_predicted ~ Years * Race + slope_change_years * Race +
    age_group + Insurance + prior_modulator + isOnEnzymes + (1 | eDWID),
  data = encounter
)
# Model plot
pred <- predict(change_mod, re.form = NA, newdata = pop_dat, se.fit = T)
...
# Fit mixed model with change point
change_mod <- lmer(
  bmipercentile ~ Years * Race + slope_change_years * Race + age_group +
    Insurance + prior_modulator + isOnEnzymes + (1 | eDWID),
...
```{r}
change_mod <- lmer(
  bmivalue ~ Years * Race + slope_change_years * Race + age_group +
    Insurance + prior_modulator + isOnEnzymes + (1 | eDWID),
  data = encounter[encounter$age_eti_start >= 20, ]
)
# Model plot
...
# Fit mixed model with change point
change_mod <- lmer(
  heightpercentile ~ Years * Race + slope_change_years * Race + age_group +
    Insurance + prior_modulator + isOnEnzymes + (1 | eDWID),
  data = encounter[encounter$age_eti_start < 20, ]
)
# Model plot
...
```{r}
change_mod <- lmer(
  height ~ Years * Race + slope_change_years * Race + age_group +
    Insurance + prior_modulator + isOnEnzymes + (1 | eDWID),
  data = encounter[encounter$age_eti_start >= 20, ]
)
# Model plot
...
# Fit mixed model with change point
change_mod <- lmer(
  weightpercentile ~ Years * Race + slope_change_years * Race + age_group +
    Insurance + prior_modulator + isOnEnzymes + (1 | eDWID),
  data = encounter[encounter$age_eti_start < 20, ]
)
# Model plot
...
```{r}
change_mod <- lmer(
  weight ~ Years * Race + slope_change_years * Race + age_group +
    Insurance + prior_modulator + isOnEnzymes + (1 | eDWID),
  data = encounter[encounter$age_eti_start >= 20, ]
)
# Model plot
